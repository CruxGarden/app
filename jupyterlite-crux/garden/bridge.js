...
export async function startGarden() {
  ...
  async function settle() {
    ...
    while (app?.busy()) {
      if (Date.now() > deadline)
        throw new Error('The notebook is still running. Keep this editor open.');
      await new Promise((resolve) => setTimeout(resolve, 40));
    }
  }
  async function capture() {
    const project = structuredClone(await app.capture());
    ...
    for (const file of project.files) {
      if (file.type !== 'directory') file.content = await encode(file.content);
    }
    const doc = { version: 1, app: 'jupyterlite', project };
    validateProject(doc);
  ...
  async function decodeProject(project) {
    ...
    for (const file of project.files) {
      if (file.type !== 'directory') file.content = await decode(file.content);
    }
    return project;
  }
  function save() {
