...
function App() {
  ...
  useEffect(() => {
    if (window.rawGarden) return // The Crux document owns startup state.
  ...
  const importProject = useCallback(
    async (project, fromUrl) => {
      ...
      if (project.currentChart.rawCustomChart) {
        if (window.rawGarden)
          throw new Error('This Garden edition uses the bundled chart catalog.')
        try {
      ...
      await hydrateFromSavedProject(project)
      ...
      <div className="app-sections">
        ...
        {data && (
          <Section title="2. Choose a chart">
            ...
            <ChartSelector
              onAddChartClick={
                window.rawGarden ? undefined : toggleModalCustomChart
              }
