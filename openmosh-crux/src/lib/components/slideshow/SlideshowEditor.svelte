<script lang="ts">
	...
	import { registerGardenEditor } from "../../../garden/bridge";
	...
	function saveSlideshowSession(strict = false) {
	...
	onMount(() =>
		registerGardenEditor({
			...
			async flush() {
				await tick();
				clearTimeout(sessionSaveTimer);
				await saveSlideshowSession(true);
			},
			inspect: () => ({
				mode: "slideshow",
				config: $state.snapshot(config),
				slides: slides.map((s) => ({ id: s.id, name: s.file.name })),
			}),
			async command() {
				throw new Error(
					"Effect commands currently work in Single and Editor modes. Use the native slideshow controls here.",
				);
			},
		}),
	);
