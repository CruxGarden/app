<script lang="ts">
	import { inGarden, registerGardenEditor } from "../../../garden/bridge";
	import { changeEffect, inspectEffects } from "../../../garden/effects";
	...
	let openingSources: Promise<void> = Promise.resolve();
	onMount(() => {
		openingSources = (async () => {
	...
	function saveSingleSession(strict = false) {
		if (isSequenceMode) return;
		...
		if (!inGarden && !moshSession.touched && !hasText && !hasMedia) return;
		const source = file;
		const state = {
		...
		return saveSession("single", [source, ...layerFiles], state, currentTrackId)
			.then(() => pruneSequenceMedia())
			.catch((e) => {
				if (strict) throw e;
	...
	onMount(() =>
		registerGardenEditor({
			async flush() {
				await tick();
				await openingSources;
				if (sourceRegistry.loadingTotal > 0)
					throw new Error(
						"Media is still loading. Save again when the import finishes.",
					);
				if (isSequenceMode) {
					// The standalone editor intentionally keeps its primary file ephemeral.
					// A complete Garden project needs every original, including that file.
					await putSequenceMedia(
				...
				clearTimeout(sessionSaveTimer);
				const saved = await flushSequenceSave();
				if (saved === false)
					throw new Error("OpenMosh could not save the timeline.");
				await flushMediaPoolSave();
				await saveSingleSession(true);
			},
			stop: pauseTrack,
			inspect: () => ({
				mode: isSequenceMode ? "editor" : "single",
				...inspectEffects(getPanelEffects()),
				file: file.name,
			}),
			async command(command) {
				if (isSequenceMode && (panelNoTarget || panelRolledNote))
					throw new Error("Select a static timeline segment or FX clip first.");
				const updated = changeEffect(getPanelEffects(), command);
				pauseTrack();
				panelBeforeEdit();
				setPanelEffects(updated);
				markPanelSegmentEdited();
				endPanelBurst();
				await tick();
				return inspectEffects(getPanelEffects());
			},
			frame: () =>
				new Promise<Blob>((resolve, reject) => {
					if (!canvasEl || noSequenceMedia) {
						reject(new Error("Add media before saving a frame."));
						return;
					}
					captureAtOutputRes(0, (done) =>
						canvasEl!.toBlob((blob) => {
							...
							else reject(new Error("Unable to export the frame."));
						}, "image/png"),
					);
				}),
		}),
