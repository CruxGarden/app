...
function NewObjectDialog({
  ...
  const [currentTab, setCurrentTab] = React.useState(
    window.gardenGDevelop ? 'new-object' : getNewObjectDialogDefaultTab()
  );
  ...
  const onObjectTypeSelected = React.useCallback(
    (enumeratedObjectMetadata: ObjectShortHeader) => {
      if (enumeratedObjectMetadata.assetStoreTag && !window.gardenGDevelop) {
          ...
          <Dialog
            ...
            fixedContent={
              <Tabs
                ...
                ].filter(option => !window.gardenGDevelop || option.value === 'new-object')}
