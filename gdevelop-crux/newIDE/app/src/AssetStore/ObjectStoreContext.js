...
export const ObjectStoreContext: React.Context<ObjectStoreState> =
  React.createContext<ObjectStoreState>({
    filters: null,
    searchResults: null,
    fetchObjects: () => {},
    error: null,
...
export const ObjectStoreStateProvider = ({
  ...
  const [installedObjectMetadataList, setInstalledObjectMetadataList] =
  ...
  const fetchObjects = React.useCallback(() => {
    if (window.gardenGDevelop) return;
    ...
    (async () => {
      ...
      try {
        ...
        objectShortHeaders.forEach((objectShortHeader) => {
          ...
          translatedObjectShortHeader.requiredExtensions =
            objectShortHeader.requiredExtensions
