...
export const buildMainMenuDeclarativeTemplate = ({
  ...
  const fileTemplate: MenuDeclarativeItemTemplate = {
    ...
    submenu: [
      ...
      {
        label: i18n._(t`Export web game...`),
...
export const adaptFromDeclarativeTemplate = (
  ...
  const adaptMenuDeclarativeItemTemplate = (
    ...
    menuTemplate.filter(item => !window.gardenGDevelop || !['main-menu-show-version-history', 'main-menu-invite-collaborators', 'main-menu-open-profile', 'main-menu-open-ask-ai'].includes(item.onClickSendEvent)).map((menuItemTemplate: MenuDeclarativeItemTemplate) => {
      const {
