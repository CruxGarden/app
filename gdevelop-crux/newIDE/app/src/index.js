...
import { memoryStorage } from '../../../garden/model.mjs';
...
class Bootstrapper extends Component<{}, State> {
  ...
  async componentDidMount() {
    try {
      ...
      const memory = memoryStorage(garden.initial?.preferences || {}, () => garden.changed());
      Object.defineProperty(window, 'localStorage', { configurable: true, value: memory.storage });
      window.gardenGDevelop = { garden, memory };
    } catch (error) { this.handleEditorLoadError(error); return; }
    ...
    loadScript(
      ...
      initializeGDevelopJs({
        ...
        // Garden provides Collaboration; upstream analytics are disabled.

        if (electron) {
          import(/* webpackChunkName: "local-app" */ './LocalApp')
            .then(async module =>
              this.setState({
                App: await module.create(this.authentication),
                loadingMessage: '',
              })
            )
