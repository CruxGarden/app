...
## Consistent source adaptation

Follow the root `EMBEDDED-APP-INTEGRATION-CONTRACT.md`. New or deepened native bridges use `embedded-apps/shared/command-session.js` for serialized settling, validation and confirmed saves. Each editor still owns its native actions, history and capture. The build copies the helper into each adopting fork's `garden/shared/`; package that source as well as the runtime so an exported Crux remains independently rebuildable. PPTist and miniPaint are the first adopters. Record narrow upstream hooks in each `UPSTREAM.md` and verify their native workflows after version updates. Generic app-tool registration alone is not proof of useful agent coverage.
