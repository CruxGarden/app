...
for (const name of readdirSync(join(root, 'runtime'), { recursive: true }).filter((n) => n.endsWith('.map'))) {
  let map;
  try { map = JSON.parse(readFileSync(join(root, 'runtime', name), 'utf8')); } catch { continue; }
...
let text = 'PPTist local build dependency notices.\nUpstream PPTist is licensed under the AGPL-3.0: see ../LICENSE. Modified source, the pinned lockfile and UPSTREAM.md accompany this runtime.\n';
