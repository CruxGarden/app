...
if (!window.CRUX_GARDEN && Blockbench.startup_count == 1) {
	try {
		jQuery.ajax({
...
if (!window.CRUX_GARDEN && Blockbench.startup_count == 3) {
	try {
		jQuery.ajax({
...
if (!window.CRUX_GARDEN) AutoBackup.initialize();
...
(function() {
	...
	function proceed() {
		...
		if (isApp) {
			...
			if (!window.CRUX_GARDEN) loadInfoFromURL();
		}
		proceeded = true;
	}
	(window.CRUX_GARDEN ? Promise.resolve() : loadInstalledPlugins()).then(proceed);
