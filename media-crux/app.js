...
(function () {
  ...
  var BUILT_IN = [
    ...
    },
    {
      ...
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}.webm',
      args: ['-y', '-i', '{in}', '-c:v', 'libvpx-vp9', '-crf', '32', '-b:v', '0', '{out}'],
    },
    {
      id: 'to-gif',
      name: 'To GIF (480 wide)',
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}.gif',
      args: [
        '-y',
        '-i',
        '{in}',
    ...
    {
      id: 'small',
      name: 'Smaller (720 wide)',
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}-720.mp4',
      args: [
        '-y',
        '-i',
        '{in}',
    ...
    {
      id: 'first-10',
      name: 'First ten seconds',
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}-10s.mp4',
      args: [
        '-y',
        '-i',
        '{in}',
    ...
    {
      id: 'frames',
      name: 'Extract frames (2 a second)',
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}-frames/f%04d.png',
    ...
    },
    {
      ...
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}-silent.mp4',
      args: ['-y', '-i', '{in}', '-an', '-c:v', 'copy', '{out}'],
    },
    {
      id: 'pull-audio',
      name: 'Take the audio out',
      tool: 'ffmpeg',
      accepts: VIDEO,
      out: 'exports/{name}.m4a',
    ...
    },
    {
      ...
      tool: 'ffmpeg',
      accepts: AUDIO,
      out: 'exports/{name}.mp3',
      args: ['-y', '-i', '{in}', '-c:a', 'libmp3lame', '-q:a', '2', '{out}'],
    },
    {
      id: 'normalise',
      name: 'Even out the loudness',
      tool: 'ffmpeg',
      accepts: AUDIO,
      out: 'exports/{name}-level.m4a',
      args: [
        '-y',
        '-i',
        '{in}',
    ...
    },
    {
      ...
      tool: 'ffmpeg',
      accepts: AUDIO,
      out: 'exports/{name}-mono.m4a',
      args: ['-y', '-i', '{in}', '-ac', '1', '-c:a', 'aac', '-b:a', '128k', '{out}'],
    },

    {
      id: 'to-jpg',
      name: 'To JPEG (quality 85)',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}.jpg',
      args: ['{in}', '-quality', '85', '{out}'],
      alt: { tool: 'ffmpeg', args: ['-y', '-i', '{in}', '-q:v', '3', '{out}'] },
    },
    {
      id: 'to-png',
      name: 'To PNG',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}.png',
      args: ['{in}', '{out}'],
      alt: { tool: 'ffmpeg', args: ['-y', '-i', '{in}', '{out}'] },
    },
    {
      id: 'to-webp',
      name: 'To WebP',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}.webp',
      args: ['{in}', '-quality', '82', '{out}'],
      alt: { tool: 'ffmpeg', args: ['-y', '-i', '{in}', '-quality', '82', '{out}'] },
    },
    {
      id: 'resize-1200',
      name: 'Resize to 1200 wide',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}-1200.jpg',
      args: ['{in}', '-resize', '1200x', '-quality', '85', '{out}'],
      alt: {
        tool: 'ffmpeg',
        args: ['-y', '-i', '{in}', '-vf', 'scale=1200:-2', '-q:v', '3', '{out}'],
      },
    },
    {
      id: 'thumb',
      name: 'Square thumbnail (512)',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}-thumb.jpg',
      args: [
        '{in}',
        '-resize',
        '512x512^',
    ...
    },
    {
      ...
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/{name}-grey.jpg',
      args: ['{in}', '-colorspace', 'Gray', '-quality', '88', '{out}'],
      alt: {
        tool: 'ffmpeg',
        args: ['-y', '-i', '{in}', '-vf', 'format=gray', '-q:v', '3', '{out}'],
      },
    },
    {
      id: 'favicon',
      name: 'Favicon (.ico, four sizes)',
      tool: 'magick',
      accepts: IMAGE,
      out: 'exports/favicon.ico',
    ...
    {
      id: 'to-html',
      name: 'To a standalone page',
      tool: 'pandoc',
      accepts: DOC,
      out: 'exports/{name}.html',
      args: ['{in}', '--standalone', '-o', '{out}'],
    },
    {
      id: 'to-epub',
      name: 'To EPUB',
  ...
  function have(tool) {
    // Not a binary: Pandoc writes the page and Crux Garden prints it, so this
    // works wherever Pandoc does — and on an HTML file, with nothing at all.
    if (tool === 'pdf') return true;
    var t = state.tools.filter(function (x) {
      return x.tool === tool;
    })[0];
    return !!(t && t.path);
  }
  ...
  // Fetching a tool the app does not carry. It can take minutes and it can
  // fail honestly ("Homebrew is not on this machine, here is the command"),
  // so the page says what is happening and asks for the tools again after.
  function install(tool) {
    if (state.busy) return;
    state.busy = true;
    renderRecipes();
    say(
      'Installing ' + (tool === 'magick' ? 'ImageMagick' : tool) + '. This can take a few minutes…',
    );
    garden
      .ask({ type: 'crux:media:install', tool: tool })
      .then(function (answer) {
        say(
          (answer && answer.message) || 'The install finished.',
          answer && answer.ok ? 'ok' : 'warn',
        );
        return garden.ask({ type: 'crux:media:tools', refresh: true });
      })
      .then(function (list) {
        state.tools = list || [];
        renderTools();
      ...
      .catch(function (error) {
        say(
          'The install did not finish — ' + (error && error.message ? error.message : error),
          'warn',
        );
      })
      .then(function () {
  ...
  function renderTools() {
    ...
    $('tools').innerHTML = state.tools
      .map(function (t) {
        ...
        // A tool the app can fetch offers to do it, here, where you noticed
        // it was missing.
        var install =
          !t.path && t.installable
            ? ' <button type="button" class="chip-install" data-install="' +
              esc(t.tool) +
              '">Install</button>'
            : '';
        return (
      ...
      .join('');
    var buttons = $('tools').querySelectorAll('[data-install]');
    for (var i = 0; i < buttons.length; i++)
      buttons[i].addEventListener('click', function (event) {
        install(event.target.getAttribute('data-install'));
      });
  ...
  function renderFiles() {
    ...
    order.forEach(function (k) {
      ...
      groups[k].forEach(function (f) {
        html +=
          '<li><button type="button" data-path="' +
          esc(f.path) +
          '"' +
  ...
  function loadFilesUntil(path, tries) {
    return loadFiles().then(function () {
      var there = state.files.some(function (f) {
        return f.path === path;
      });
  ...
  function loadFiles() {
    return garden.files().then(
      function (list) {
        state.files = (list || []).filter(function (f) {
          return kindOf(f.path) !== 'other' || /^exports\//.test(f.path);
        });
        renderFiles();
      },
      function (err) {
  ...
  function loadRecipes() {
    return garden.read('data/project.json').then(
      function (text) {
        ...
        var byId = {};
        BUILT_IN.concat(own).forEach(function (r) {
          if (r && r.id) byId[r.id] = r;
        });
        state.recipes = Object.keys(byId).map(function (k) {
          return byId[k];
        });
      },
      function () {
        state.recipes = BUILT_IN.slice();
      },
    );
  }

  // A recipe names the tool it prefers; some also name a second route for a
  // machine that does not have it (the picture recipes go through ffmpeg,
  // which ships with the app, when ImageMagick is absent). Everything the
  // page runs goes through here, so what is offered is what can actually run.
  function resolved(r) {
    if (have(r.tool)) return r;
    if (r.alt && have(r.alt.tool))
      return {
        id: r.id,
        name: r.name,
        accepts: r.accepts,
  ...
  function accepts(r, path) {
    return Array.isArray(r.accepts) && r.accepts.indexOf(ext(path)) >= 0;
  }

  function recipesFor(path) {
    return state.recipes
      .filter(function (r) {
        return accepts(r, path);
      })
      .map(resolved)
      .filter(Boolean);
  }
  ...
  function renderRecipes() {
    ...
    var missing = state.recipes.filter(function (r) {
      return accepts(r, path) && !resolved(r);
    });
    ...
    if (!fits.length) {
      ...
      $('command').textContent = missing.length
        ? 'Nothing to run: ' +
          missing.length +
          ' recipe(s) for this kind need ' +
          (missing[0].tool === 'magick' ? 'ImageMagick' : missing[0].tool) +
  ...
  function chosenRecipe() {
    var id = $('recipe').value;
    var r = state.recipes.filter(function (x) {
      return x.id === id;
    })[0];
  ...
  function showCommand() {
    ...
    var args = (r.args || []).map(function (a) {
      return fill(a, state.chosen, out);
    });
    $('command').hidden = false;
  ...
  function runRecipe() {
    ...
    var args = (r.args || []).map(function (a) {
      return fill(a, path, out);
    });
    setBusy(true);
    say('Running ' + r.name + '…');
    remember('pending', JSON.stringify({ name: r.name, out: out, path: path, at: Date.now() }));
    var started = Date.now();
    // `pdf` is not a binary: Pandoc writes the page and Crux Garden prints it.
    var run =
      r.tool === 'pdf'
        ? garden.pdf(path, out).then(function (made) {
            return { code: 0, ms: Date.now() - started, stdout: '', path: made && made.path };
          })
        : garden.run(r.tool, args);
    run.then(
      function (res) {
        setBusy(false);
        ...
        if (res.code === 0) {
          say(
            r.name + ' → ' + out + '  (' + secs + ' s)' + (res.stdout ? '\n\n' + res.stdout : ''),
            'good',
          );
          appendLog(
            '- `' + out + '` — ' + r.name + ' from `' + path + '` (' + r.tool + ', ' + secs + ' s)',
          );
        ...
        if (!info) {
          say(
            'Could not read ' +
              ...
              ' — ' +
              (have('magick')
                ? 'the tool answered nothing'
                : 'ImageMagick is not on this machine') +
              '.',
          );
          return;
        }
            ...
            if (s.codec_type === 'video')
              rows.push([
                ...
                s.codec_name +
                  ...
                  '×' +
                  s.height +
                  (s.r_frame_rate ? ' @ ' + s.r_frame_rate.replace('/1', '') : ''),
              ]);
  ...
  if (!garden.inGarden) {
    ...
    Promise.all([loadTools(false), loadRecipes()]).then(function () {
      loadFiles().then(function () {
        ...
        var stillThere = state.files.some(function (f) {
          return f.path === last;
        });
