...
export async function startGarden() {
  ...
  async function settle() {
    ...
    while (app?.busy()) {
      if (Date.now() > deadline)
        throw new Error('Wait for the board operation to finish before closing this editor.');
  ...
  async function capture() {
    ...
    validateProject({ version: 1, app: 'kan', project });
    ...
    const doc = { version: 1, app: 'kan', project };
    validateProject(doc);
  ...
  function save(commitInputs = false) {
    ...
    const operation = tail.then(async () => {
      clearTimeout(timer);
      try {
        await app?.beforeSave(commitInputs);
  ...
  bar.querySelectorAll('button')[1].onclick = () => {
    ...
    dialog.setAttribute('aria-label', 'Discard unsaved board changes?');
    ...
    dialog.innerHTML =
      '<p>Discard unsaved board changes and reopen the saved project?</p><button>Keep editing</button> <button>Discard and reload</button>';
  ...
  try {
    ...
    validateProject({ version: 1, app: 'kan', project: initial });
    return {
      initial,
      flush: () => save(true),
      registerDraft(flush) {
