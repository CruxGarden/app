...
export async function startGarden() {
	...
	style.textContent =
		"#garden-project{position:fixed;bottom:0;left:0;right:0;height:32px;z-index:10000;display:flex;gap:12px;align-items:center;padding:0 10px;background:#24282c;color:#fff;font:12px system-ui}#garden-project span{flex:1}#garden-project button{padding:3px 8px;color:#fff;background:#42494f;border:1px solid #697078;border-radius:3px}#root{height:calc(100dvh - 34px)!important;overflow:auto}#root .h-screen{height:calc(100dvh - 34px)!important}#root .min-h-screen{min-height:calc(100dvh - 34px)!important}";
	document.head.append(style);
	document.body.append(bar);
	const cover = document.createElement("div");
	cover.style.cssText =
		"position:fixed;inset:0 0 34px;z-index:9999;background:#fafafacc;cursor:wait";
	...
	async function settle() {
		...
		while (app?.busy()) {
			if (Date.now() > deadline)
				throw new Error(
					"Wait for the video operation to finish before closing this editor.",
				);
	...
	async function capture() {
		...
		validateProject({ version: 1, app: "opencut", project });
		...
		const doc = { version: 1, app: "opencut", project };
		validateProject(doc);
	...
	function save(commitInputs = false) {
		// Explicit close/export/agent actions commit native on-blur fields.
		// Background autosave must never take focus from someone typing.
		if (commitInputs && document.activeElement instanceof HTMLElement)
	...
	async function command(value) {
		if (hydrating || !app)
			throw new Error("Wait for the video editor to open.");
		await save(true);
	...
	bar.querySelectorAll("button")[0].onclick = () => save(true).catch(() => {});
	bar.querySelectorAll("button")[1].onclick = () => {
		if (revision === saved) {
			window.location.reload();
			return;
		}
		if (document.querySelector("#garden-discard")) return;
		const dialog = document.createElement("dialog");
		dialog.id = "garden-discard";
		dialog.setAttribute("aria-label", "Discard unsaved video changes?");
		dialog.style.cssText =
			"background:#24282c;color:white;border:1px solid #697078;border-radius:8px;padding:20px;font:14px system-ui;max-width:380px";
		dialog.innerHTML =
			"<p>Discard unsaved video changes and reopen the saved project?</p><button>Keep editing</button> <button>Discard and reload</button>";
		dialog.querySelectorAll("button")[0].onclick = () => dialog.close();
	...
	try {
		...
		validateProject({ version: 1, app: "opencut", project: initial });
		return {
			initial,
			flush: () => save(true),
			registerDraft(flush) {
