...
import { flushGarden } from "../../../../../garden/state";
...
function ProjectDropdown() {
	...
	const handleExit = async () => {
		...
		try {
            ...
            toast.error("Your draft is still open", { description: error instanceof Error ? error.message : "Save the project before leaving." });
        } finally {
            setIsExiting(false);
        }
	};
...
function EditableProjectName() {
    ...
    useEffect(() => {
        if (!isEditing && inputRef.current) inputRef.current.value = projectName;
	...
	const saveEdit = async () => {
		if (!isEditing || !inputRef.current || !activeProject) return;
