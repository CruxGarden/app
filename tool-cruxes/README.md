...
Six built-in Cruxes demonstrate manual creation and optional agent assistance:

| Add Crux         | Category                | Editable document                                    | Export                   |
| ---------------- | ----------------------- | ---------------------------------------------------- | ------------------------ |
| Whiteboard       | Drawing / diagrams      | Native Excalidraw scene and separate images          | Drawing JSON, PNG, SVG   |
| Spreadsheet      | Business / productivity | Native Univer workbook, formulas and formatting      | Workbook JSON, sheet CSV |
...
Native editor capture and calculation behavior is documented in `../docs/productivity-tools/README.md`. Excalidraw 0.18.1 and Univer 0.25.1 are pinned; package/license supplements and local font notices are incorporated by `scripts/productivity-build.mjs`. Bundling fails if a contributing package lacks a license text. The fallback Liberation font is upgraded to OFL-licensed 2.1.5; its checked-in WOFF2 conversion and source hash are in `license-supplements/`.
