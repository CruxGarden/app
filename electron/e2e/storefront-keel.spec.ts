...
/**
 * The Storefront starter (Keel + a products collection): a person adds a product
 * from the Builder and the live site serves it at /shop/<slug> with its price and
 * buy button; the scripted collaborator adds a product with write_file; Share
 * builds and publishes the shop with search, feed and social cards; the shop
 * survives a restart; a complete Crux archive imports cleanly.
 */
const evidence = resolve(__dirname, '../../docs/storefront-keel');

test('Storefront: Builder product, live route with price, collaborator product, share, restart, clean import', async () => {
  test.setTimeout(20 * 60_000);
  mkdirSync(evidence, { recursive: true });
  const api = await startMockApi();
  const first = await launchApp({ env: { CRUX_AI_MOCK: '1', CRUX_API_URL: api.url } });
  const archive = join(first.dir, 'shop.crux');
  let folder = '';
  let id = '';
  const post = (name: string) => join(folder, 'src/content/products', `${name}.md`);
  ...
  try {
    ...
    await test.step('create from the picker; the Builder lists the seed post', async () => {
      ...
      await expect.poll(() => existsSync(post('garden-print')), { timeout: 60000 }).toBe(true);
      ...
      await page.screenshot({ path: join(evidence, 'shop-builder.png') });
    });
    ...
    await test.step('a person writes a post; the live site serves and lists it', async () => {
      ...
      await expect
        ...
        .toBe('ready');
      await expect(preview).toHaveAttribute('src', /\/shop\/compost$/);
      siteUrl = (await preview.getAttribute('src'))!;
      await page.screenshot({ path: join(evidence, 'shop-preview.png') });
      ...
      try {
        ...
        await expect(site.locator('[data-buy]')).toBeVisible();
        await site.goto(new URL('/', siteUrl).toString());
        await expect(site.getByRole('link', { name: /Compost/ })).toBeVisible();
        await site.screenshot({ path: join(evidence, 'shop-front.png'), fullPage: true });
        await site.goto(new URL('/shop/', siteUrl).toString());
    ...
    await test.step('the scripted collaborator drafts a post with write_file', async () => {
      ...
      await expect(
        page.getByText('Added the moss jar to the shop at 18 dollars.', {
          exact: true,
        }),
      ).toBeVisible({
        timeout: 120000,
      });
      await expect.poll(() => existsSync(post('moss-jar')), { timeout: 30000 }).toBe(true);
      const { site, close } = await openSite();
      try {
        await expect
          .poll(
            async () =>
              (await site.request.get(new URL('/shop/moss-jar/', siteUrl).toString())).status(),
            {
              timeout: 60000,
            },
          )
          .toBe(200);
        await site.goto(new URL('/shop/moss-jar/', siteUrl).toString());
        await expect(site.getByRole('heading', { name: 'Moss jar' })).toBeVisible();
        await expect(site.locator('[data-buy]')).toContainText('18');
        await site.screenshot({ path: join(evidence, 'shop-agent.png') });
      } finally {
        await close();
      }
      await collab.click();
    ...
    await test.step('Share builds the site and publishes it with search and a feed', async () => {
      ...
      for (const path of [
        'index.html',
        'shop/compost/index.html',
        'shop/moss-jar/index.html',
        'shop/index.html',
  ...
  try {
    ...
    await test.step('clean Garden: the complete Crux imports with every post', async () => {
      ...
      await expect.poll(() => existsSync(post('moss-jar')), { timeout: 60000 }).toBe(true);
      expect(existsSync(post('compost'))).toBe(true);
      expect(existsSync(join(folder, 'src/content/products/moss-tea.md'))).toBe(true);
      expect(existsSync(join(folder, 'node_modules'))).toBe(false);
      await page.screenshot({ path: join(evidence, 'shop-imported.png') });
    });
  } finally {
