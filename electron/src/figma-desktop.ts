...
// A conservative POC: one standard Figma window, with identity/geometry checked
// again before each mutation. Never guess which of several windows to move.
const SCRIPT = String.raw`
function run(argv) {
  var input = JSON.parse(argv[0]);
  var se = Application('System Events');
  var ps = se.applicationProcesses.whose({bundleIdentifier: 'com.figma.Desktop'})();
  if (ps.length !== 1) throw new Error('Open Figma, then retry.');
  var p = ps[0];
  var windows = p.windows().filter(function(w) { return w.subrole() === 'AXStandardWindow'; });
  if (windows.length !== 1) throw new Error('Keep one Figma window open for automatic placement, or arrange manually.');
  var w = windows[0];
  try { if (w.attributes.byName('AXFullScreen').value()) throw new Error('Leave Figma full screen before arranging.'); }
  catch (e) { if (String(e).indexOf('Leave Figma') !== -1) throw e; }
  function snapshot() {
  ...
  if (!expected || before.pid !== expected.pid || before.title !== expected.title ||
    ...
    throw new Error('The Figma window changed. It was left in place; arrange it manually or retry.');
  var b = input.bounds;
...
export class FigmaDesktop {
  ...
  async open() {
    await exec('/usr/bin/open', ['-b', 'com.figma.Desktop'], { timeout: 10000 });
  }
  status() {
    return {
  ...
  async arrange(side: 'left' | 'right') {
    ...
    try {
      ...
      await this.open();
      const figma = await command({ action: 'inspect' });
      const garden = win.getBounds();
      const minimum = win.getMinimumSize();
      const arranged = await command({ action: 'set', expected: figma, bounds: layout.figma });
      ...
      if (
        ...
        throw new Error(
          'Figma could not fit the requested space. Its layout was restored; arrange manually.',
        );
      }
      try {
