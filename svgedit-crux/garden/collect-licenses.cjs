...
// The canvas workspace is prebuilt before the editor, so its dependency
// sources are not all visible in the editor's final sourcemaps.
const {execFileSync} = require('node:child_process');
for (const dir of execFileSync('npm', ['ls', '--omit=dev', '--all', '--parseable'], {cwd:root, encoding:'utf8'}).trim().split('\n')) {
  const file = path.join(dir, 'package.json');
  if (fs.existsSync(file)) packages.set(dir, JSON.parse(fs.readFileSync(file, 'utf8')));
}
let text = 'Dependency notices for the SVG-Edit browser build.\n\n';
...
console.log(
	`Included notices for ${packages.size} bundled dependency packages.`
);
