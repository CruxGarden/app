...
export async function startGarden() {
	...
	async function settle() {
		...
		while (app?.busy()) {
			if (Date.now() > deadline)
				throw new Error(
					'Wait for the drawing operation to finish before closing this editor.'
				);
	...
	async function capture() {
		...
		for (const key of Object.keys(project))
			project[key] = await encode(project[key]);
	...
	async function decodeProject(project) {
		...
		for (const key of Object.keys(project))
			project[key] = await decode(project[key]);
		return project;
	}
	function save() {
	...
	async function command(value) {
		if (hydrating || !app)
			throw new Error('Wait for the drawing editor to open.');
		await save();
