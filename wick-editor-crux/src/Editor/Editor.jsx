...
class Editor extends EditorCore {
  ...
  UNSAFE_componentWillMount = () => {
    ...
    // Leave Page warning.
    // Crux Garden: the host owns unsaved-state warnings, and its thumbnail-capture windows load this
    // page standalone, so the leave-page prompt is only installed outside the Garden's Electron shell.
    if (window.parent === window && !/Electron/.test(navigator.userAgent)) window.onbeforeunload = function(event) {
  ...
  componentDidMount = () => {
    ...
    this.hidePreloader();
    this.onWindowResize();
    // Crux Garden: inside a Crux the saved project opens instead of URL/autosave lookups.
    gardenBoot(this).then((owned) => {
      if (owned) return;
