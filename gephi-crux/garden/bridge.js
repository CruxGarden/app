...
export async function startGarden() {
  ...
  async function settle() {
    ...
    while (app?.busy()) {
      if (Date.now() > deadline) throw new Error("Stop the running graph layout before closing this editor.");
  ...
  async function capture() {
    ...
    const doc = { version: 1, app: "gephi", project };
    validateProject(doc);
  ...
  async function command(value) {
    if (hydrating || !app) throw new Error("Wait for the graph app to open.");
    await save();
