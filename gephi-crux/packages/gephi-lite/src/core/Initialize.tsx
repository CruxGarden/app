...
import { connectGardenGraph } from "../garden-native";
...
export const Initialize: FC<PropsWithChildren<unknown>> = ({ children }) => {
  ...
  const initialize = useCallback(async () => {
    ...
    if (window.gardenGraph) url.search = "";
    ...
    if (window.gardenGraph) {
      connectGardenGraph();
      if (window.gardenGraph.initial) showWelcomeModal = false;
    }
  ...
  useEffect(() => {
    initialize().catch((error) => {
      console.error(error);
      window.gardenGraph?.failed(error);
      notify({
        type: "error",
        title: t("error.title"),
