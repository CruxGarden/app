...
> {
  ...
  if (firstLine?.startsWith("<?xml")) {
    ...
    // Graphology JSON is ordinary graph data. Decode it before the SDK reviver
    // creates its own Graph instance (which can be from a different version).
    const plain = JSON.parse(fileContent);
    if (Array.isArray(plain?.nodes) && Array.isArray(plain?.edges)) {
      return { format: "graphology", data: Graph.from(plain) };
    }
