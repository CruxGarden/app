...
function envOrDefault(envVar: string | undefined, fallback: string): string {
  ...
  // A relative value (the offline copy under public/wasm in the Crux Garden build) resolves against the page.
  if (/^[a-z][a-z0-9+.-]*:/i.test(envVar) || typeof document === 'undefined') return envVar;
  return new URL(envVar, document.baseURI).href;
}
