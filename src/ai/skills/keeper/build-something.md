...
4. **What exists already?** Files, a brief, a name, a look, anything to bring in (they can drop files into a crux; you can list_cruxes).
...
- plant_crux for each member into that Cruxspace: a clear title, the right template ("blank" for a page or an app the collaborator will write; "notes-app" for writing; "astro-homepage" for a site; a tool id the garden has installed for anything else), and a brief that says what this crux is for and what to make first.
...
- Keep a short running account in this conversation: which member, what it made, what is left.
...
- Close with what exists now: the Cruxspace, its members, the address, and that every step is kept — the person can export the Cruxspace as one package and "How was this made?" will show all of it.
