...
- `leaderboard:<YYYY-MM-DD>` — mode `public`: the crux's one value for the day, `{ entries: [{ name, score, seconds, at }] }`; anyone reads it, writing (like every store write) needs a sign-in. The page maintains it — read, replace-or-add the visitor's entry, sort (score, then seconds, then post time), cap at 50, write back — under its own convention, which the store does not enforce: one entry per name, the name being the signed-in account's username. Top 20 on the shelf page, top 10 after the reveal.
