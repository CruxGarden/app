...
- `src/pages/index.astro` (the shelf, bounded and visible), `src/pages/rounds/[slug].astro` (a transcript), `src/pages/about.astro`, `src/lib/shelf.ts` (the question fallback by kind). The site's heading is always the shelf's `question` — never hardcode a product name into the pages.
