...
export function useStoreApiProxy(cruxId: string | null) {
  useEffect(() => {
    ...
    function handleMessage(e: MessageEvent) {
      ...
      switch (type) {
        case 'crux:store:get':
          client.get(`/store/${cruxId}/${encodeURIComponent(key)}`)
            .then((r) => reply(e.source, 'crux:store:get:res', id, { value: r.data?.value }))
            .catch(() => reply(e.source, 'crux:store:get:res', id, { value: null }));
          break;

        case 'crux:store:set':
          client.put(`/store/${cruxId}/${encodeURIComponent(key)}`, { value, mode: mode || 'protected' })
            .catch(() => {});
          break;

        case 'crux:store:inc':
          client.post(`/store/${cruxId}/${encodeURIComponent(key)}/inc`, { by: by ?? 1 })
            .then((r) => reply(e.source, 'crux:store:inc:res', id, { value: r.data?.value }))
            .catch(() => reply(e.source, 'crux:store:inc:res', id, { value: 0 }));
          break;

        case 'crux:store:del':
          client.delete(`/store/${cruxId}/${encodeURIComponent(key)}`)
            .catch(() => {});
          break;

        case 'crux:store:list':
          client.get(`/store/${cruxId}`)
