...
/** People, not bytes: every visitor the crux has had, counted once a day each (edge logs, hashed per day). */
function Audience({ usage }: { usage: usageApi.CruxUsage }) {
  return (
    <div className="flex items-baseline justify-between gap-2 text-xxs" data-testid="crux-audience">
      <span className="text-text">Visitors</span>
      <span className="font-mono text-text-muted">
        {usage.visitors > 0 ? (
          <>
            <span className="text-text">{usage.visitors.toLocaleString()}</span> all time
          </>
        ) : (
          'none counted yet'
        )}
