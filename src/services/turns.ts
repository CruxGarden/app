...
import { delegateFor } from './delegate';
...
export function turnsFor(store: StoreApi<CruxState>) {
  ...
  if (!turns) {
    turns = createTurns(store);
    controllers.set(store, turns);
  }
  return turns;
}
export function useTurns() {
  return turnsFor(useCruxStoreApi());
}
function createTurns(useCruxStore: StoreApi<CruxState>) {
  const { delegateTasks } = delegateFor(useCruxStore);
  let starting = false;
  ...
  async function submitTurn(content: string): Promise<void> {
    const s = useCruxStore.getState();
    if (!s.crux || s.closing) return;
    if (starting || isJobActive(s.turnJob) || activeRuns.has(s.crux.id)) {
  ...
  async function removeQueued(index: number): Promise<void> {
    const s = useCruxStore.getState();
  ...
  async function runNextQueued(): Promise<void> {
    const s = useCruxStore.getState();
    if (!s.crux || s.closing || starting || isJobActive(s.turnJob) || activeRuns.has(s.crux.id))
      return;
    ...
    const wantsCheck =
      ...
      (isFixTurn ||
        shouldAutoCheck({
          ...
          content,
          mutated,
          visual: isVisualCrux(live.workspaceArtifacts ?? live.artifacts),
  ...
  async function runCheckCycle(args: {
    ...
    try {
      const deps = await defaultCheckDeps({ workspace: useCruxStore, model, apiKey });
      outcome = await runCheck(
        {
          ...
          plan,
          reply,
          siteCrux: isSiteCrux(live.workspaceArtifacts ?? live.artifacts),
   ...
   */
  async function runParallelJob(prompt: string, tasks: SubagentTask[]): Promise<void> {
    const s = useCruxStore.getState();
    const crux = s.crux;
    if (
      !crux ||
      ...
      starting ||
      tasks.length === 0 ||
      isJobActive(s.turnJob) ||
      activeRuns.has(crux.id)
    )
      return;
    const cruxId = crux.id;
  ...
  async function setVerifyOnDone(enabled: boolean): Promise<void> {
    const s = useCruxStore.getState();
    if (!s.crux) return;
  ...
  return {
    submitTurn: trackWorkspaceOperation(useCruxStore, submitTurn),
    stopTurn,
    steerTurn: trackWorkspaceOperation(useCruxStore, steerTurn),
    ...
    setVerifyOnDone: trackWorkspaceOperation(useCruxStore, setVerifyOnDone),
    drain: async () => {
      while (activeRuns.size || starting) {
        await Promise.all([...activeRuns.values()]);
        if (starting) await new Promise((r) => setTimeout(r, 10));
      }
    },
  };
}
