...
const delegates = new WeakMap<StoreApi<CruxState>, ReturnType<typeof createDelegate>>();
export function delegateFor(data: StoreApi<CruxState>) {
  let delegate = delegates.get(data);
  if (!delegate) {
    delegate = createDelegate(data);
...
function createDelegate(useCruxStore: StoreApi<CruxState>) {
    ...
    const finalRuns = await runSubagents(tasks, runs, {
      ...
      converse: (task, i, childSignal) => {
        ...
        const guarded: typeof exec = async (name, input) =>
          childSignal.aborted
            ? 'Error: the worker was stopped before this ran.'
            : exec(name, input);
        const system =
  ...
  async function chooseConflict(path: string, choice: number | 'keep'): Promise<void> {
