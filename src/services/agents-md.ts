...
function renderConventions(site: boolean, model: ContentModel | undefined): string {
  ...
  lines.push(
    "- Media, documents and pictures convert in place: `run_ffmpeg`, `run_magick` (ImageMagick) and `run_pandoc` take arguments only — no program name, no shell, paths relative to this folder and inside it. `probe_media` says what a file is before you choose them; `media_tools` says which binaries this machine has (ImageMagick and Pandoc are often the system's and may be missing — say so rather than promising). Outputs land beside the source or in `exports/`, never over the source.",
