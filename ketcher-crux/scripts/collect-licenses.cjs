...
// Indigo is embedded inside the upstream standalone distribution, so its
// package path does not necessarily survive that distribution's source maps.
const indigoDir = path.join(root, 'node_modules/indigo-ketcher');
packages.set(indigoDir, JSON.parse(fs.readFileSync(path.join(indigoDir, 'package.json'), 'utf8')));
let text = 'Dependency notices for the Ketcher browser build.\n\n';
