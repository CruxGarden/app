...
describe('canvasNodeSemantics', () => {
  ...
  it('exposes icon components as whole-control link targets', () => {
    ...
    expect(linkableElementsForNode(icon)).toEqual([{ key: 'whole', label: 'Whole Control' }]);
    expect(linkableElementsForNode(iconText)).toEqual([{ key: 'whole', label: 'Whole Control' }]);
  });

  it('exposes icon components to generic text styling controls', () => {
    expect(nodePropertyCapabilities(createCanvasNode('icon', 0, 0, 'icon')).genericTextStyle).toBe(
      true,
    );
    expect(
      nodePropertyCapabilities(createCanvasNode('iconText', 0, 0, 'icon-text')).genericTextStyle,
    ).toBe(true);
  });
  ...
  it('describes common property capabilities for multi-selection', () => {
    ...
    expect(
      nodesShareGenericPaintControls([button, createCanvasNode('textLabel', 0, 0, 'label')]),
    ).toBe(false);
  });
