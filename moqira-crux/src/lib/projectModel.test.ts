...
describe('projectModel', () => {
  ...
  it('duplicates a Wireframe with copied Canvas Nodes at the original positions and default fallbacks', () => {
    const source = {
      ...wireframe('source', 'Checkout', [node('node-a')]),
  ...
  it('groups Project history changes by group key and supports undo and redo', () => {
    ...
    const first = commitProjectHistoryChange(
      initialHistory,
      renameProject('First'),
      { groupKey: 'name' },
      null,
    );
    const second = commitProjectHistoryChange(
      first.history,
      renameProject('Second'),
      { groupKey: 'name' },
      first.groupKey,
    );
    const third = commitProjectHistoryChange(
      second.history,
      renameProject('Third'),
      { groupKey: 'other' },
      second.groupKey,
