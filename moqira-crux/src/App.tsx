...
import { useGardenProject, GardenControls } from './GardenProject';
import './styles/app.css';
import './garden.css';
import { open as openDialog, save as saveDialog } from './lib/native';
...
function activeTextInput() {
  ...
  if (lastTextInput?.isConnected && lastTextInput.closest('.floating-text-editor'))
...
function runEditableClipboardAction(
  ...
  if (!element) {
    ...
    const selectedText = lastTextInputSelection
      ? lastTextInputSelection.value.slice(
          lastTextInputSelection.selectionStart,
          lastTextInputSelection.selectionEnd,
        )
      : '';
  ...
  const selectionEnd =
    element.selectionEnd ?? lastTextInputSelection?.selectionEnd ?? selectionStart;
  ...
  if (action === 'paste') {
    void navigator.clipboard
      ?.readText()
      .then((text) => replaceSelectedText(element, text, 'insertFromPaste'));
    return true;
  }
  const selectedText = element.value.slice(selectionStart, selectionEnd);
  if (!selectedText) return true;
  void navigator.clipboard?.writeText(selectedText);
  if (action === 'cut' && selectionStart !== selectionEnd)
    replaceSelectedText(element, '', 'deleteByCut');
  return true;
}
...
function imageSizeFromDataUrl(dataUrl: string): Promise<{ width: number; height: number }> {
  return new Promise((resolve) => {
    ...
    image.onload = () =>
      resolve({ width: image.naturalWidth || 320, height: image.naturalHeight || 240 });
...
function imageBlobFromDataTransfer(
...
function projectNameFromPath(path: string) {
  return (
    decodeTitleFromFilename(fileNameFromPath(path).replace(/\.(moq|moqira|dsmockup|json)$/i, '')) ||
    'Untitled Project'
  );
}
...
function defaultSaveFileName(project: MockupProject, projectPath: string | null) {
  ...
  const baseName =
    project.name.trim() && project.name !== 'New Project'
      ? project.name.trim()
      : 'Untitled Project';
  return `${encodeTitleForFilename(baseName)}.moq`;
}

function App({ publicProject }: { publicProject?: MockupProject }) {
  const readOnly = !!publicProject;
  const [projectPath, setProjectPath] = useState<string | null>(null);
  const [projectHistory, setProjectHistory] = useState<ProjectHistory>(() =>
    createProjectHistory(publicProject ?? createDefaultProject(createId)),
  );
  const [appAppearance, setAppAppearance] = useState<MockupProject['appearance']>(() => {
    if (publicProject) return publicProject.appearance;
    const appearance = defaultAppearance();
    appearance.colorScheme =
      (readStoredValue(themeKey, legacyThemeKey) as MockupProject['appearance']['colorScheme']) ||
      appearance.colorScheme;
    appearance.accentColor = readStoredValue(accentKey, legacyAccentKey) || appearance.accentColor;
    appearance.appFontFamily =
      readStoredValue(appFontFamilyKey, legacyAppFontFamilyKey) || appearance.appFontFamily;
    appearance.appFontSize =
      Number(readStoredValue(appFontSizeKey, legacyAppFontSizeKey)) || appearance.appFontSize;
    appearance.accentTitlebar =
      readStoredValue(accentTitlebarKey, legacyAccentTitlebarKey) === 'true';
    return appearance;
  });
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [clipboard, setClipboard] = useState<CanvasNode[]>([]);
  const [contextMenu, setContextMenu] = useState<ContextMenuState | null>(null);
  const [wireframeContextMenu, setWireframeContextMenu] =
    useState<WireframeContextMenuState | null>(null);
  ...
  const [interactiveMode, setInteractiveMode] = useState(readOnly);
  const [leftCollapsed, setLeftCollapsed] = useState<boolean>(
    () => localStorage.getItem(leftPaneCollapsedKey) === 'true',
  );
  const [rightCollapsed, setRightCollapsed] = useState<boolean>(
    () => localStorage.getItem(rightPaneCollapsedKey) === 'true',
  );
  ...
  const activeWireframe = useMemo(
    () =>
      project.wireframes.find((wireframe) => wireframe.id === project.activeWireframeId) ??
  ...
  const visibleComponentLibrary = useMemo(() => {
    const definitions = controlCatalogue.filter(
      (definition) =>
        activeComponentCategory === 'All' ||
        componentCategoryNames(definition).includes(activeComponentCategory),
    );
    if (activeComponentCategory === 'All')
    ...
    const matches = query
      ? controlCatalogue
          .map((definition, index) => ({
            definition,
            index,
            score: quickAccessScore(definition, query),
          }))
  ...
  const resetProjectHistory = useCallback(
    (nextProject: MockupProject, saved = true) => {
  ...
  const commitProjectChange = useCallback(
    (updater: (project: MockupProject) => MockupProject, options: ProjectChangeOptions = {}) => {
      ...
      setProjectHistory((current) => {
        const result = commitProjectHistoryChange(
          current,
          updater,
          options,
          activeProjectHistoryGroupKeyRef.current,
        );
  ...
  useEffect(() => {
    ...
    Promise.resolve(readStoredValue(projectPathKey, legacyProjectPathKey))
      ...
      .then((rememberedPath) => {
        ...
        return openProjectFile(rememberedPath).then((loadedProject) => ({
          rememberedPath,
          loadedProject,
        }));
      })
      .then((loadedProject) => {
        ...
        setRecentProjects((current) =>
          addRecentProject(current, loadedProject.rememberedPath, projectName),
        );
      })
      .catch((error) => {
    ...
    (kind: ComponentKind, x?: number, y?: number) => {
      ...
      const point =
        x === undefined || y === undefined
          ? visibleCanvasInsertionPoint(node.width, node.height)
  ...
  const addImageNode = useCallback(
    (image: ClipboardImage, x?: number, y?: number) => {
      ...
      const point =
        x === undefined || y === undefined
          ? pastePointForSize(size.width, size.height)
          : { x: Math.round(x), y: Math.round(y) };
  ...
  const previewNodes = useCallback(
  ...
  const beginTextEdit = useCallback(
    (node: CanvasNode) => {
      ...
      const height = multiline
        ? clamp(Math.max(170, node.height + 82, naturalEditorHeight), 170, maxEditorHeight)
        : 96;
  ...
  const closeTextEditor = useCallback(
    (commit: boolean, draftOverride?: string) => {
      ...
      if (!editor) return;
      if (commit) {
        const node = project.wireframes
          .flatMap((wireframe) => wireframe.nodes)
          .find((item) => item.id === editor.nodeId);
        if (!node) {
          textEditorRef.current = null;
          setTextEditor(null);
          return;
  ...
  const garden = useGardenProject({
    project,
    readOnly,
    load: resetProjectHistory,
    capture: () => {
      flushSync(() => closeTextEditor(true));
    },
    saved: (saved) => {
      savedProjectSnapshotRef.current = dirtyProjectSnapshot(saved);
      setProjectPath('project.json');
    },
  });
  ...
  const deleteNode = useCallback(
    (id?: string | null) => {
      ...
      mutateActiveWireframe((wireframe) => ({
        ...wireframe,
        nodes: wireframe.nodes.filter((node) => !deleted.has(node.id)),
      }));
  ...
  const pasteClipboardNodes = useCallback(
    (x?: number, y?: number) => {
      ...
      const nodes = cloneNodesForPaste(
        clipboard,
        createId,
  ...
  useEffect(() => {
    const onPaste = (event: ClipboardEvent) => {
      ...
      event.preventDefault();
      pendingCanvasPasteRef.current = null;
      void clipboardImageFromBlob(imageBlob.blob, imageBlob.mimeType).then((image) =>
        addImageNode(image),
      );
    };
  ...
  const layerNode = useCallback(
    (id: string | null, action: LayerAction) => {
      ...
      mutateActiveWireframe((wireframe) => ({
        ...wireframe,
        nodes: moveNodeLayer(wireframe.nodes, ids, action),
      }));
    },
    [mutateActiveWireframe, selectedIds],
  );
  ...
  const lockNode = useCallback(
    (id?: string | null) => {
      ...
      mutateActiveWireframe((wireframe) => ({
        ...wireframe,
        nodes: wireframe.nodes.map((node) =>
          locked.has(node.id) ? { ...node, locked: true } : node,
        ),
      }));
    },
    [mutateActiveWireframe, selectedIds],
  ...
  const selectWireframe = useCallback(
    (wireframeId: string) => {
      if (readOnly) {
        setProjectHistory((current) => ({
          ...current,
          present: { ...current.present, activeWireframeId: wireframeId },
        }));
        return;
      }
      endProjectHistoryGroup();
  ...
  const followLink = useCallback(
    (link: CanvasLink | undefined) => {
      ...
      if (link.kind === 'back') {
        ...
        if (
          previousWireframeId &&
          project.wireframes.some((wireframe) => wireframe.id === previousWireframeId)
        ) {
      ...
      if (project.activeWireframeId)
        wireframeNavigationStackRef.current.push(project.activeWireframeId);
  ...
  useEffect(() => {
    ...
    addMenuListener('menu-open-project', () =>
      runAfterMenuCloses(() => void menuActionsRef.current.openProject()),
    );
    addMenuListener('menu-save-project', () => void menuActionsRef.current.saveProject(false));
    addMenuListener('menu-save-project-as', () =>
      runAfterMenuCloses(() => void menuActionsRef.current.saveProject(true)),
    );
    ...
    addMenuListener<string>('menu-open-recent-project', (event) =>
      menuActionsRef.current.openRecentProject(event.payload),
    );
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (dragState.kind === 'move') {
            ...
            return position
              ? [
                  [
                    id,
                    {
                      x: Math.max(0, position.x + snap.deltaX),
                      y: Math.max(0, position.y + snap.deltaY),
                    },
                  ],
                ]
              : [];
        ...
        setDragState({
          ...dragState,
          currentX: originalPosition.x + snap.deltaX,
          currentY: originalPosition.y + snap.deltaY,
        });
        ...
        setDragState({
          ...dragState,
          ...nextBounds,
          currentX: nextBounds.x,
          currentY: nextBounds.y,
          currentWidth: nextBounds.width,
          currentHeight: nextBounds.height,
        });
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      const moved =
        paletteDrag.moved ||
        Math.hypot(event.clientX - paletteDrag.startX, event.clientY - paletteDrag.startY) > 4;
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      const moved =
        selectionRect.moved ||
        Math.hypot(point.x - selectionRect.startX, point.y - selectionRect.startY) > 4;
    ...
    const onPointerUp = () => {
      ...
      if (selectionRect.moved) {
        const rect = rectFromPoints(
          ...
          selectionRect.currentX,
          selectionRect.currentY,
        );
        const hits = (activeWireframe?.nodes ?? [])
          .filter((node) => rectContainsNode(rect, node))
          .map((node) => node.id);
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      const point = canvasPointFromEvent(event);
      const moved =
        arrowDraw.moved || Math.hypot(point.x - arrowDraw.startX, point.y - arrowDraw.startY) > 4;
    ...
    const onPointerUp = () => {
      ...
      if (
        arrowDraw.moved &&
        Math.hypot(arrowDraw.currentX - arrowDraw.startX, arrowDraw.currentY - arrowDraw.startY) > 8
      ) {
  ...
  const deleteWireframe = (wireframeId = activeWireframe?.id) => {
    ...
    mutateProject((current) => {
      ...
      const activeWireframeId =
        current.activeWireframeId === wireframeId
          ? nextWireframes[0].id
  ...
  const commitRenameWireframe = (wireframeId: string, name: string) => {
    ...
    const duplicate = project.wireframes.some(
      (wireframe) =>
        wireframe.id !== wireframeId &&
        wireframe.name.trim().toLowerCase() === nextName.toLowerCase(),
    );
    if (duplicate) return false;
    mutateProject((current) => ({
      ...current,
      wireframes: current.wireframes.map((wireframe) =>
        wireframe.id === wireframeId ? { ...wireframe, name: nextName } : wireframe,
      ),
    }));
    return true;
  };
  ...
  const openCanvasContextMenu = (event: React.MouseEvent) => {
    ...
    if (interactiveMode) return;
    const point = canvasPointFromEvent(event);
    const stack = [...(activeWireframe?.nodes ?? [])]
      .filter((node) => pointHitsNode(point.x, point.y, node))
      .reverse();
    const selected = new Set(selectedIds);
  ...
  const contextTargetName = contextMenu?.targetId
    ...
    : 'Canvas';
  const contextActionTargetId =
    contextMenu?.targetId && selectedIds.includes(contextMenu.targetId)
      ? null
  ...
  return (
    <div
      data-public-edition={readOnly || undefined}
      className={interactiveMode ? 'app-shell links-active is-interactive' : 'app-shell'}
    >
      {!readOnly && <GardenControls garden={garden} project={project} />}
      <header
            ...
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === 'ArrowDown') {
                  event.preventDefault();
                  setQuickAccessOpen(true);
                  setQuickAccessIndex((index) =>
                    clamp(index + 1, 0, Math.max(quickAccessMatches.length - 1, 0)),
                  );
                }
      ...
      <main
        className={`workspace${leftCollapsed ? ' left-collapsed' : ''}${effectiveRightCollapsed ? ' right-collapsed' : ''}`}
      >
        ...
        {leftCollapsed ? null : (
          <aside className="left-pane">
            ...
            >
              {project.wireframes.map((wireframe) => (
                <button
                  ...
                  type="button"
                  className={
                    wireframe.id === activeWireframe?.id
                      ? 'wireframe-row is-active'
                      : 'wireframe-row'
                  }
            ...
            <div className="wireframe-actions">
              <button type="button" onClick={() => duplicateWireframe()}>
                Duplicate
              </button>
              <button
                type="button"
                onClick={() => deleteWireframe()}
                disabled={project.wireframes.length === 1}
              >
                Delete
              </button>
            </div>
        ...
        <section className="center-pane">
          ...
          >
            <div
              ...
              style={
                {
                  fontFamily: project.appearance.appFontFamily,
                  fontSize: project.appearance.appFontSize,
                  color: '#17202f',
                  '--accent': project.appearance.accentColor,
                  '--app-font-family': project.appearance.appFontFamily,
                  '--app-font-size': `${project.appearance.appFontSize}px`,
                  '--text': '#17202f',
                  '--muted': '#667085',
                } as CSSProperties
              }
            ...
            >
              ...
              {selectionRect?.moved ? (
                <div
                  ...
                  style={{
                    ...
                    width: rectFromPoints(
                      selectionRect.startX,
                      selectionRect.startY,
                      selectionRect.currentX,
                      selectionRect.currentY,
                    ).width,
                    height: rectFromPoints(
                      selectionRect.startX,
                      selectionRect.startY,
                      selectionRect.currentX,
                      selectionRect.currentY,
              ...
              {arrowDraw?.moved ? <ArrowDrawPreview state={arrowDraw} /> : null}
              {activeWireframe?.nodes.map((node) => (
                <CanvasItem
                  ...
                  interactiveSelect={
                    interactiveSelect?.nodeId === node.id ? interactiveSelect : null
                  }
                  onSelect={(additive) => {
                  ...
                  onMoveStart={(event) => {
                    ...
                    const movingNodes =
                      activeWireframe?.nodes.filter(
                        (item) => nodeIds.includes(item.id) && !item.locked,
                      ) ?? [];
                    setDragState({
                      kind: 'move',
      ...
      {wireframeContextMenu ? (
        <WireframeContextMenu
          ...
          onNew={addWireframe}
          onRename={() =>
            wireframeContextMenu.wireframeId &&
            beginRenameWireframe(wireframeContextMenu.wireframeId)
          }
          onDuplicate={() =>
            wireframeContextMenu.wireframeId && duplicateWireframe(wireframeContextMenu.wireframeId)
          }
          onDelete={() =>
            wireframeContextMenu.wireframeId && deleteWireframe(wireframeContextMenu.wireframeId)
          }
        />
      ) : null}
      {renameWireframe ? (
        <RenameWireframeDialog
          ...
          onChange={(draft) =>
            setRenameWireframe((current) => (current ? { ...current, draft } : current))
          }
          onCancel={() => setRenameWireframe(null)}
          onSave={() => {
            if (!commitRenameWireframe(renameWireframe.wireframeId, renameWireframe.draft))
              return false;
      ...
      {textEditor ? (
        <FloatingTextEditor
          ...
          onFormatChange={(patch) =>
            updateNode(textEditor.nodeId, patch, { groupKey: `text-format:${textEditor.nodeId}` })
          }
      ...
      {settingsOpen ? (
        <SettingsDialog
          ...
          onAppearanceChange={(appearance) =>
            setAppAppearance((current) => ({ ...current, ...appearance }))
          }
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        if (editingLocked) {
          if (linksActive && (event.target as HTMLElement).closest('[data-link-key]'))
            event.stopPropagation();
          return;
        }
      ...
      onClick={(event) => {
        ...
        const accordionTarget = (event.target as HTMLElement).closest<HTMLElement>(
    ...
    >
      <div className="canvas-node-clip">
        <NodeContent
          ...
          selectedOptionIndex={interactiveSelect?.selectedIndex ?? null}
        />
      </div>
      {interactiveSelect?.open && (node.kind === 'dropdown' || node.kind === 'comboBox') ? (
        <div
          className="interactive-select-menu"
        ...
        >
          {nodeOptions(node, ['First', 'Second', 'Third']).map((option, index) => (
            <button
              key={`${option}-${index}`}
              type="button"
              className={textFormatClassName(
                node,
                index === interactiveSelect.selectedIndex ? 'is-selected' : '',
              )}
              onClick={() => onInteractiveOptionSelect(index)}
            >
...
function ArrowSelectionHandles({
  node,
  onResizeStart,
}: {
  node: CanvasNode;
...
function parseAccordionSections(node: CanvasNode): AccordionSection[] {
  ...
  nodeOptions(node, ['Item One', 'Item Two', 'Item Three', 'Item Four']).forEach(
    (raw, optionIndex) => {
      const childMatch = raw.match(/^\s*-\s*(.*)$/);
      if (childMatch && sections.length) {
        sections[sections.length - 1].children.push({
          text: childMatch[1] || raw.trim(),
          optionIndex,
          raw,
        });
        return;
...
function accordionOpenIndex(node: CanvasNode, selectedOptionIndex?: number | null) {
  ...
  const requestedIndex =
    typeof selectedOptionIndex === 'number' ? selectedOptionIndex : node.activeIndex;
...
function NodeContent({
  ...
  if (
    ['button', 'circleButton', 'pointyButton', 'multilineButton', 'helpButton'].includes(node.kind)
  )
    return <ButtonVisual node={node} />;
  if (
    [
      'text',
      'textLabel',
  ...
  if (
    ...
    return <MediaVisual node={node} />;
  if (node.kind === 'stickyNote')
    return (
      <div className={textFormatClassName(node, 'editable-node-text')}>
        {renderInlineFormatting(node.text ?? '')}
      </div>
    );
  return null;
}
...
function TextVisual({
  ...
  return (
    <div className={textClassName} data-link-key="whole">
      {renderInlineMarkdown(text, { selected, linksActive, onLinkClick })}
    </div>
  );
}
...
function FormVisual({
        ...
        {nodeOptions(node, ['Option one', 'Option two', 'Option three']).map((option, index) => {
          ...
          return (
            <div
              key={`${option}-${index}`}
              className={
                checkboxRow.kind === 'checkbox' && checkboxRow.disabled ? 'is-disabled' : ''
              }
            ...
            >
              {node.kind === 'radioButtonGroup' ? (
                <span className={index === 0 ? 'radio-dot is-checked' : 'radio-dot'} />
              ) : null}
              {node.kind === 'checkboxList' && checkboxRow.kind === 'checkbox' ? (
                <span
                  className={`mock-checkbox${checkboxRow.checked ? ' is-checked' : ''}${checkboxRow.indeterminate ? ' is-indeterminate' : ''}`}
                />
              ) : null}
              <span className={textFormatClassName(node)}>
                {renderInlineFormatting(checkboxRow.text)}
              </span>
            </div>
          );
        })}
  ...
  if (node.kind === 'dropdown' || node.kind === 'comboBox') {
    ...
    const storedIndex =
      typeof node.activeIndex === 'number'
        ? clamp(node.activeIndex, 0, Math.max(0, options.length - 1))
        : null;
    ...
    return (
      <div className="dropdown-node">
        <span className={textFormatClassName(node)}>
          {renderInlineFormatting(selectedOption ?? node.text ?? '')}
        </span>
        <ChevronDown size={16} />
      </div>
    );
  }
  if (node.kind === 'textbox' || node.kind === 'textInput')
    return (
      <div className={textFormatClassName(node, 'textbox-node')}>
        {renderInlineFormatting(node.text || node.placeholder || '')}
      </div>
    );
  if (node.kind === 'textArea')
    return (
      <div className={textFormatClassName(node, 'textarea-node')}>
        {renderInlineFormatting(node.text ?? '')}
      </div>
    );
  ...
  if (node.kind === 'colorPicker')
    return (
      <div className="color-picker-node">
        <span />
      </div>
    );
  if (node.kind === 'numericStepper')
    return (
  ...
  if (node.kind === 'onOffSwitch')
    return (
      <div className={node.checked ? 'switch-node is-on' : 'switch-node'}>
        <span />
      </div>
    );
  if (node.kind === 'progressBar' || node.kind === 'progressBarIndeterminate') {
    return (
      <div className="progress-node">
        <span
          style={{
            width: node.kind === 'progressBarIndeterminate' ? '28%' : `${nodePercent(node)}%`,
          }}
        />
      </div>
    );
...
function NavigationVisual({
  ...
  if (node.kind === 'buttonBar') {
    ...
    return (
      <Segmented
        ...
        linksActive={linksActive}
        onLinkClick={onLinkClick}
        textClassName={textFormatClassName(node)}
      />
    );
  }
  if (node.kind === 'vTabs')
    return (
      <div className="v-tabs-node">
        {nodeOptions(node).map((item, index) => (
          <LinkedVisualItem
            ...
            className={textFormatClassName(
              node,
              index === (node.activeIndex ?? 0) ? 'is-active' : '',
            )}
          >
      ...
      <div className={`linkbar-node ${node.kind}`}>
        {nodeOptions(node).map((item, index) => (
          <LinkedVisualItem
            ...
            linksActive={linksActive}
            onLinkClick={onLinkClick}
            className={textFormatClassName(node)}
          >
            {node.kind === 'breadcrumbs' ? (
              <span className="breadcrumb-label">{renderInlineFormatting(item)}</span>
            ) : (
              renderInlineFormatting(item)
            )}
          </LinkedVisualItem>
  ...
  if (node.kind === 'playback')
    return (
      <div className="playback-node">
        <span>◀◀</span>
        <span>▶</span>
        <span>▶▶</span>
      </div>
...
function AppBarVisual({
  ...
  return (
    <div className="app-bar-node">
      {iconAt(0, 'Menu')}
      <span className={textFormatClassName(node, 'app-bar-title')}>
        {renderInlineFormatting(node.text ?? 'Heading')}
      </span>
...
function TabsVisual({
  ...
  return (
    <div
      className={`tabs-node tabs-${placement} tabs-align-${alignment}${node.showBorder === false ? ' no-border' : ' has-border'}${node.showScrollbar ? ' has-scrollbar' : ''}${node.textBold ? ' tabs-text-bold' : ''}${node.textItalic ? ' tabs-text-italic' : ''}${node.textUnderline ? ' tabs-text-underline' : ''}${node.textStrikethrough ? ' tabs-text-strikethrough' : ''}`}
    >
...
function ContainerVisual({
  ...
  if (node.kind === 'fieldSet')
    return (
      <fieldset className="fieldset-node">
        <legend className={textFormatClassName(node)}>
          {renderInlineFormatting(node.text ?? '')}
        </legend>
      </fieldset>
    );
  if (node.kind === 'popover')
    return (
      <div className={textFormatClassName(node, 'popover-node')}>
        <span />
        {renderInlineFormatting(node.text ?? '')}
      </div>
    );
  if (node.kind === 'tooltip')
    return (
      <div className={textFormatClassName(node, 'tooltip-node')}>
        {renderInlineFormatting(node.text ?? '')}
      </div>
    );
  if (node.kind === 'callout')
    return (
      <div className={textFormatClassName(node, 'callout-node')}>
        {renderInlineFormatting(node.text ?? '')}
      </div>
    );
  return null;
  ...
  return (
    <div className={className}>
      {sections.map((section) => {
        ...
        return (
          <div
            key={`${section.raw}-${section.optionIndex}`}
            className={isOpen ? 'accordion-section is-open' : 'accordion-section'}
          >
            <span
...
function ChromeFrame({ node }: { node: CanvasNode }) {
  return (
    <div
      className={`chrome-frame-node${node.showBorder === false ? ' no-border' : ''}${node.showScrollbar ? ' has-scrollbar' : ''}`}
    >
      <div>
        <span />
        <span />
        <span />
        <strong className={textFormatClassName(node)}>
          {renderInlineFormatting((node.text ?? '').split('\n')[0] ?? '')}
        </strong>
      </div>
      <section>{node.showScrollbar ? <i aria-hidden="true" /> : null}</section>
    </div>
  );
}
...
function DataVisual({
  ...
  if (node.kind === 'dateChooser')
    return (
      <div className={textFormatClassName(node, 'date-chooser-node')}>
        {renderInlineFormatting(node.text ?? '')}
        <span>▣</span>
      </div>
    );
  if (node.kind === 'timePicker')
    return (
  ...
  if (node.kind === 'streetMap')
    return (
      <div className="street-map-node">
        <span />
        <span />
        <span />
      </div>
    );
  if (node.kind === 'tagCloud')
    return (
      <div className="tag-cloud-node">
        {(node.text ?? '').split(/\s+/).map((word, index) => (
          <span key={`${word}-${index}`} className={textFormatClassName(node)}>
            {renderInlineFormatting(word)}
          </span>
        ))}
      </div>
    );
    ...
    <div className="tree-pane-node">
      {parseTreePaneRows(node).map((row) => (
        ...
        >
          ...
          <span className={textFormatClassName(node, 'tree-pane-label')}>
            {renderInlineFormatting(row.label)}
          </span>
        </span>
      ))}
    </div>
...
function TreePaneIcon({ icon }: { icon: TreePaneRow['icon'] }) {
  if (icon === 'none')
    return <span className="tree-pane-icon tree-pane-icon-none" aria-hidden="true" />;
        ...
        {grid.rows.map((row, rowIndex) => (
          <tr key={`${row.join('|')}-${rowIndex}`}>
            {grid.columns.map((column, index) => (
              <td
                key={index}
                className={parseDataGridControl((row[index] ?? '').trim()) ? 'is-control-cell' : ''}
...
function parseDataGrid(node: CanvasNode): { columns: ParsedDataGridColumn[]; rows: string[][] } {
  const source = node.text?.trim()
    ...
    : [
        ...
        ...(node.rows?.length
          ? node.rows.map((row) => row.split('|').join(', '))
          : ['Alice, PM, Active', 'Ben, Design, Review']),
      ].join('\n');
  const lines = source
    .split(/\r?\n/)
    .map((line) => line.trimEnd())
    .filter((line) => line.trim());
  const specMatch = lines
    .at(-1)
    ?.trim()
    .match(/^\{(.+)\}$/);
...
function parseDataGridColumnSpecs(
  spec: string,
): Array<{ align?: 'left' | 'center' | 'right'; width?: string }> {
  const parts = parseDelimitedRow(spec);
  const numericParts = parts.map((part) => Number(part.match(/\d+/)?.[0] ?? 1));
  const positiveTotal = numericParts
...
function createDataGridBlankRows(
  node: CanvasNode,
  grid: { columns: ParsedDataGridColumn[]; rows: string[][] },
) {
  ...
  const headerHeight = Math.ceil(
    Math.max(...grid.columns.map((column) => dataGridLineCount(column.text)), 1) * lineHeight +
      cellVerticalPadding,
  );
  const bodyHeight = grid.rows.reduce((height, row) => {
    const rowLines = Math.max(
      ...grid.columns.map((_, index) => dataGridLineCount(row[index] ?? '')),
      1,
    );
  ...
  return Array.from(
    { length: Math.max(0, Math.floor(extraHeight / rowHeight) - 1) },
    (_, index) => `blank-${index}`,
  );
}
...
function parseDataGridControl(
  cell: string,
): { kind: 'checkbox' | 'radio'; state: 'empty' | 'checked' | 'mixed' } | null {
  ...
  return (
    <div className="calendar-node">
      ...
      {Array.from({ length: 35 }, (_, index) => (
        <span key={index}>{index > 4 ? index - 4 : ''}</span>
      ))}
    </div>
  );
}
...
function SiteMapVisual({ node }: { node: CanvasNode }) {
  ...
  return (
    <div className="site-map-node">
      <strong className={textFormatClassName(node)}>
        {renderInlineFormatting(items[0] ?? '')}
      </strong>
      {items.slice(1).map((item) => (
        <span key={item} className={textFormatClassName(node)}>
          {renderInlineFormatting(item)}
        </span>
      ))}
    </div>
...
function ChartVisual({ node }: { node: CanvasNode }) {
  ...
  if (node.kind === 'chartLine')
    return (
      <div className="chart-line-node">
        <span />
        <span />
        <span />
      </div>
    );
  if (node.kind === 'chartBar' || node.kind === 'chartColumn')
    return (
      <div className={`chart-bars-node ${node.kind}`}>
        {[58, 82, 42, 68].map((value) => (
          <span key={value} style={{ '--bar-value': `${value}%` } as React.CSSProperties} />
        ))}
      </div>
    );
  if (node.kind === 'hScrollBar' || node.kind === 'vScrollBar')
    return (
      <div className={`scrollbar-node ${node.kind}`}>
        <span className="scrollbar-button scrollbar-button-start" />
        <span className="scrollbar-track" />
        <span className="scrollbar-thumb" />
        <span className="scrollbar-button scrollbar-button-end" />
      </div>
    );
  if (node.kind === 'hSlider' || node.kind === 'vSlider' || node.kind === 'volumeSlider')
    return (
      <div className={`slider-node ${node.kind}`}>
        <span />
        <i style={{ '--slider-value': `${nodePercent(node)}%` } as React.CSSProperties} />
      </div>
    );
  return null;
}
...
function arrowControlPoint(node: CanvasNode, start: CanvasPoint, end: CanvasPoint): CanvasPoint {
  if (node.arrowControl)
    return arrowSvgPoint(node, normalizedArrowPoint(node.arrowControl, { x: 0.5, y: 0.5 }));
...
function ArrowVisual({ node }: { node: CanvasNode }) {
  ...
  return (
    <div className="arrow-node">
      ...
      {label ? (
        <div className={labelClassName} style={{ left: labelPoint.x, top: labelPoint.y }}>
...
function MarkupVisual({ node }: { node: CanvasNode }) {
  ...
  if (node.kind === 'hRule' || node.kind === 'vRule')
    return <div className={`rule-node ${node.kind}`} />;
  if (node.kind === 'hSplitter' || node.kind === 'vSplitter')
    return (
      <div className={`splitter-node ${node.kind}`}>
        <span />
      </div>
    );
  if (node.kind === 'redX')
    return (
      <div className="red-x-node">
        <span />
        <span />
      </div>
    );
  if (node.kind === 'scratchOut')
    return (
  ...
  if (node.kind === 'hCurlyBrace' || node.kind === 'vCurlyBrace')
    return (
      <div className={`curly-node ${node.kind}`}>
        <span>{node.kind === 'hCurlyBrace' ? '︷' : '}'}</span>
        <small className={textFormatClassName(node)}>
          {renderInlineFormatting(node.text ?? '')}
        </small>
      </div>
    );
...
function MediaVisual({ node }: { node: CanvasNode }) {
  if (node.kind === 'icon') {
    ...
    return (
      <Icon
        ...
        color={paint.stroke}
        fill={paint.fill}
        size={Math.max(12, Math.min(node.width, node.height) - 14)}
      />
    );
  }
  if (node.kind === 'iconText') {
    ...
    const paint = iconPaint(node);
    return (
      <div className="icon-text-node" data-link-key="whole">
        <Icon
          color={paint.stroke}
          fill={paint.fill}
          size={Math.max(22, Math.min(node.width, node.height) / 2)}
        />
        <span className={textFormatClassName(node)}>{renderInlineFormatting(node.text ?? '')}</span>
      </div>
    );
  ...
  if (node.kind === 'image') {
    if (node.imageDataUrl) {
      return (
        <img
          className="image-node image-node-bitmap"
  ...
  if (node.kind === 'coverFlow')
    return (
      <div className="coverflow-node">
        <span />
        <span />
        <span />
      </div>
    );
  if (node.kind === 'smartphone' || node.kind === 'iphone' || node.kind === 'ipad')
    return <DeviceVisual node={node} />;
  if (node.kind === 'iosKeyboard')
    return (
      <div className="ios-keyboard-node">
        {Array.from({ length: 30 }, (_, index) => (
          <span key={index}>{index === 26 ? 'space' : ''}</span>
        ))}
      </div>
    );
  if (node.kind === 'iosMenu')
    return (
      <div className="ios-menu-node">
        {nodeOptions(node).map((item) => (
          <span key={item} className={textFormatClassName(node)}>
            {renderInlineFormatting(item)}
          </span>
        ))}
  ...
  if (node.kind === 'iosPicker')
    return (
      <div className="ios-picker-node">
        {nodeOptions(node).map((item) => (
          <span key={item} className={textFormatClassName(node)}>
            {renderInlineFormatting(item)}
          </span>
...
function DeviceVisual({ node }: { node: CanvasNode }) {
  return (
    <div className={`device-node ${node.kind}`}>
      <span />
      <section />
    </div>
  );
  ...
  return (
    <div className={compact ? 'segmented compact' : 'segmented'}>
      {items.map((item, index) => (
        <span
          key={`${item}-${index}`}
          className={[index === activeIndex ? 'is-active' : '', textClassName]
            .filter(Boolean)
            .join(' ')}
...
function FloatingTextEditor({
  ...
  const baseEditorHeight = editor.multiline
    ? clamp(naturalHeight, editor.height, editor.maxHeight)
    : Math.max(editor.height, 58);
  ...
  const setTextareaSelection = (
    snapshot: Pick<TextEditSnapshot, 'selectionStart' | 'selectionEnd'>,
  ) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        width: editor.width,
        height: editorHeight,
        maxHeight: editor.maxHeight + toolbarHeight,
      }}
      onPointerDown={(event) => event.stopPropagation()}
      onClick={(event) => event.stopPropagation()}
    >
      {node ? (
        <div className="floating-text-toolbar">
          <div className="toolbar-group">
            <button
              type="button"
              className={node.textBold ? 'is-active' : ''}
              title="Bold"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => toggleFormat({ textBold: !node.textBold })}
            >
              <Bold size={16} />
            </button>
            <button
              type="button"
              className={node.textItalic ? 'is-active' : ''}
              title="Italic"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => toggleFormat({ textItalic: !node.textItalic })}
            >
              <Italic size={16} />
            </button>
            <button
              ...
              title="Underline"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => toggleFormat({ textUnderline: !textUnderline })}
            >
              <Underline size={16} />
            </button>
            <button
              type="button"
              className={node.textStrikethrough ? 'is-active' : ''}
              title="Strikethrough"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => toggleFormat({ textStrikethrough: !node.textStrikethrough })}
            >
      ...
      <textarea
        ...
        onBlur={(event) => {
          ...
          if (
            nextTarget &&
            event.currentTarget.closest('.floating-text-editor')?.contains(nextTarget)
          )
            return;
          ...
          if (modifier && ['c', 'x', 'v', 'z'].includes(key)) {
            ...
            if (key === 'z') {
              if (event.shiftKey) {
                ...
                if (!next) return;
                undoStackRef.current = [...undoStackRef.current, textInputSnapshot(textarea)].slice(
                  -100,
                );
              ...
              if (!previous) return;
              redoStackRef.current = [...redoStackRef.current, textInputSnapshot(textarea)].slice(
                -100,
              );
...
function LinkTargetSelect({
  ...
  const selectValue = value
    ? value.kind === 'wireframe'
      ? `wireframe:${value.wireframeId}`
      : value.kind
    : '';
  return (
    <select
      ...
      onChange={(event) => {
        ...
        if (!nextValue) onChange(undefined);
          ...
          const url = window.prompt(
            ...
            value?.kind === 'url' ? value.url : 'https://',
          );
        ...
        else if (nextValue.startsWith('wireframe:'))
          onChange({ kind: 'wireframe', wireframeId: nextValue.slice('wireframe:'.length) });
      }}
    >
      ...
      <section className="property-section">
        ...
        <div className="tabs-position-controls">
          <div className="toolbar-group">
            <button
              type="button"
              className={tabPlacement === 'top' ? 'is-active' : ''}
              title="Tabs on top"
              onClick={() => onChange('tabPlacement', { tabPlacement: 'top' })}
            >
              <PanelTop size={18} />
            </button>
            <button
              type="button"
              className={tabPlacement === 'bottom' ? 'is-active' : ''}
              title="Tabs on bottom"
              onClick={() => onChange('tabPlacement', { tabPlacement: 'bottom' })}
            >
              <PanelBottom size={18} />
            </button>
          </div>
          <div className="toolbar-group">
            <button
              type="button"
              className={tabAlignment === 'left' ? 'is-active' : ''}
              title="Align tabs left"
              onClick={() => onChange('tabAlignment', { tabAlignment: 'left' })}
            >
              <AlignLeft size={18} />
            </button>
            <button
              type="button"
              className={tabAlignment === 'center' ? 'is-active' : ''}
              title="Align tabs center"
              onClick={() => onChange('tabAlignment', { tabAlignment: 'center' })}
            >
              <AlignCenter size={18} />
            </button>
            <button
              type="button"
              className={tabAlignment === 'right' ? 'is-active' : ''}
              title="Align tabs right"
              onClick={() => onChange('tabAlignment', { tabAlignment: 'right' })}
            >
              <AlignRight size={18} />
            </button>
          </div>
        ...
        <div className="text-toolbar tabs-text-toolbar">
          <div className="toolbar-group">
            <button
              ...
              className={node.textBold ? 'is-active' : ''}
              title="Bold"
              onClick={() => onChange('textBold', { textBold: !node.textBold })}
            >
              <Bold size={18} />
            </button>
            <button
              ...
              className={node.textItalic ? 'is-active' : ''}
              title="Italic"
              onClick={() => onChange('textItalic', { textItalic: !node.textItalic })}
            >
              <Italic size={18} />
            </button>
            <button
              type="button"
              className={node.textUnderline ? 'is-active' : ''}
              title="Underline"
              onClick={() => onChange('textUnderline', { textUnderline: !node.textUnderline })}
            >
              <Underline size={18} />
            </button>
            <button
              ...
              title="Strikethrough"
              onClick={() =>
                onChange('textStrikethrough', { textStrikethrough: !node.textStrikethrough })
              }
            >
        ...
        <div className="text-toolbar">
          <div className="toolbar-group">
            ...
            <button
              ...
              className={textUnderline ? 'is-active' : ''}
              title="Underline"
              onClick={() => onChange('textUnderline', { textUnderline: !textUnderline })}
            >
              <Underline size={18} />
            </button>
            <button
              type="button"
              className={textStrikethrough ? 'is-active' : ''}
              title="Strikethrough"
              onClick={() =>
                onChange('textStrikethrough', { textStrikethrough: !textStrikethrough })
              }
            >
              <Strikethrough size={18} />
            </button>
        ...
        <div className="text-toolbar">
          ...
          <div className="toolbar-group">
            <button
              ...
              title="Align left"
              onClick={() => onChange('textAlign', { textAlign: 'left' })}
            >
              <AlignLeft size={18} />
            </button>
            <button
              ...
              title="Align center"
              onClick={() => onChange('textAlign', { textAlign: 'center' })}
            >
              <AlignCenter size={18} />
            </button>
            <button
              ...
              title="Align right"
              onClick={() => onChange('textAlign', { textAlign: 'right' })}
            >
              <AlignRight size={18} />
            </button>
          </div>
  ...
  return (
    <>
      <label className="property-swatch-row">
        ...
        <input
          type="color"
          value={node.fill ?? '#d9d9d9'}
          onBlur={onChangeEnd}
          onChange={(event) => onChange('fill', { fill: event.target.value })}
        />
      </label>
        ...
        <div className="appbar-icon-list">
          {icons.map((icon, index) => (
            <IconPicker
              key={`${index}-${icon}`}
              value={icon || 'Circle'}
              onChange={(name) => updateIcon(index, name)}
            />
          ))}
        </div>
      </section>
    ...
    <>
      <section className="property-section arrow-properties">
        ...
        <div className="arrow-options-grid">
          ...
          <div className="toolbar-group arrow-toolbar-group">
            <button
              type="button"
              className={line === 'curved' ? 'is-active' : ''}
              title="Curved"
              onClick={() => onChange('arrowLine', { arrowLine: 'curved' })}
            >
              <CornerUpRight size={18} />
            </button>
            <button
              type="button"
              className={line === 'straight' ? 'is-active' : ''}
              title="Straight"
              onClick={() => onChange('arrowLine', { arrowLine: 'straight' })}
            >
          ...
          <div className="toolbar-group arrow-toolbar-group">
            <button
              type="button"
              className={node.arrowHeadStart ? 'is-active' : ''}
              title="Arrowhead at start"
              onClick={() => onChange('arrowHeadStart', { arrowHeadStart: !node.arrowHeadStart })}
            >
              <ArrowLeft size={19} />
            </button>
            <button
              type="button"
              className={node.arrowHeadEnd !== false ? 'is-active' : ''}
              title="Arrowhead at end"
              onClick={() =>
                onChange('arrowHeadEnd', { arrowHeadEnd: node.arrowHeadEnd === false })
              }
            >
              <ArrowRight size={19} />
            </button>
        ...
        <label className="property-range-row">
          ...
          <input
            ...
            onBlur={onChangeEnd}
            onChange={(event) =>
              onChange('arrowLabelPosition', { arrowLabelPosition: Number(event.target.value) })
            }
          />
        </label>
      </section>
      <label className="property-swatch-row">
        ...
        <input
          ...
          onBlur={onChangeEnd}
          onChange={(event) => onChange('stroke', { stroke: event.target.value })}
        />
      </label>
      <label className="property-swatch-row">
        Label Color
        <input
          type="color"
          value={node.textColor ?? '#111827'}
          onBlur={onChangeEnd}
          onChange={(event) => onChange('textColor', { textColor: event.target.value })}
        />
      </label>
      <label className="property-range-row">
        Opacity
      ...
      <section className="property-section">
        ...
        <div className="toolbar-group arrow-stroke-toolbar">
          <button
            type="button"
            className={strokeStyle === 'solid' ? 'is-active' : ''}
            title="Solid"
            onClick={() => onChange('arrowStrokeStyle', { arrowStrokeStyle: 'solid' })}
          >
            <span className="stroke-icon stroke-solid" />
          </button>
          <button
            type="button"
            className={strokeStyle === 'dashed' ? 'is-active' : ''}
            title="Dashed"
            onClick={() => onChange('arrowStrokeStyle', { arrowStrokeStyle: 'dashed' })}
          >
            <span className="stroke-icon stroke-dashed" />
          </button>
          <button
            type="button"
            className={strokeStyle === 'dotted' ? 'is-active' : ''}
            title="Dotted"
            onClick={() => onChange('arrowStrokeStyle', { arrowStrokeStyle: 'dotted' })}
          >
...
function PropertiesPane({
  ...
  const {
    ...
    ? commonNodePropertyCapabilities(selectionNodes)
  ...
  const canChooseActiveOption =
    !isMultiSelection && hasInteractiveOptions(selectedNode) && selectedNodeOptions.length > 0;
  const selectedActiveIndex = canChooseActiveOption
    ? clamp(selectedNode.activeIndex ?? 0, 0, selectedNodeOptions.length - 1)
    : -1;
  const linkableElements = linkableElementsForNode(selectedNode);
  const changeLink = (
    key: string,
    link: CanvasLink | undefined | 'new-wireframe' | 'duplicate-wireframe',
  ) => {
    const nextLink =
      link === 'new-wireframe'
    ...
    <div className="properties">
        ...
        <div className="property-row">
          ...
          <label>
            <GeometryNumberInput
              ...
              onChange={(value) => groupedChange('x', { x: value })}
            />
            <span>X</span>
          </label>
          <label>
            <GeometryNumberInput
              ...
              onBlur={onNodeChangeEnd}
              onChange={(value) => groupedChange('y', { y: value })}
            />
            <span>Y</span>
          </label>
        </div>
        <div className="property-row">
          ...
          <label>
            <GeometryNumberInput
              ...
              onBlur={onNodeChangeEnd}
              onChange={(value) => groupedChange('width', { width: value })}
            />
            <span>Width</span>
          </label>
          <label>
            <GeometryNumberInput
              ...
              onBlur={onNodeChangeEnd}
              onChange={(value) => groupedChange('height', { height: value })}
            />
            <span>Height</span>
          </label>
        </div>
        ...
        <section className="property-section">
          ...
          <label className="toggle-setting">
            <input
              ...
              onChange={(event) =>
                groupedChange('showBorder', { showBorder: event.target.checked })
              }
            />
            <span>Show Border</span>
          </label>
        ...
        <section className="property-section">
          ...
          <label className="icon-toggle-setting" title="Show scrollbar">
            <input
              ...
              onChange={(event) =>
                groupedChange('showScrollbar', { showScrollbar: event.target.checked })
              }
            />
            <span aria-hidden="true" />
          </label>
      ...
      {showSpecializedProperties && isTabs ? (
        <>
          ...
          <label className="property-swatch-row">
            ...
            <input
              ...
              onBlur={onNodeChangeEnd}
              onChange={(event) => groupedChange('fill', { fill: event.target.value })}
            />
          </label>
          <label className="property-range-row">
            Opacity
            <input
              ...
              onBlur={onNodeChangeEnd}
              onChange={(event) =>
                groupedChange('opacity', { opacity: Number(event.target.value) })
              }
            />
          </label>
        </>
        ...
        <>
          ...
          <label>
            ...
            <input
              type="color"
              value={selectedNode.stroke ?? '#111827'}
              onBlur={onNodeChangeEnd}
              onChange={(event) => groupedChange('stroke', { stroke: event.target.value })}
            />
          </label>
        </>
      ) : null}
      {showTextColor ? (
        <label className="property-swatch-row">
          ...
          <input
            type="color"
            value={selectedNode.textColor ?? '#111827'}
            onBlur={onNodeChangeEnd}
            onChange={(event) => groupedChange('textColor', { textColor: event.target.value })}
          />
        </label>
      ) : null}
      {showGenericOpacity ? (
          ...
          {showGenericState ? (
            <section className="property-section">
              ...
              <select
                value={genericState}
                onChange={(event) =>
                  groupedChange('disabled', {
                    disabled: event.target.value === 'disabled' || undefined,
                  })
                }
              >
            ...
            <section className="property-section">
              ...
              <div className="text-toolbar">
                <div className="toolbar-group">
                  <button
                    type="button"
                    className={selectedNode.textBold ? 'is-active' : ''}
                    title="Bold"
                    onClick={() => groupedChange('textBold', { textBold: !selectedNode.textBold })}
                  >
                    <Bold size={18} />
                  </button>
                  <button
                    type="button"
                    className={selectedNode.textItalic ? 'is-active' : ''}
                    title="Italic"
                    onClick={() =>
                      groupedChange('textItalic', { textItalic: !selectedNode.textItalic })
                    }
                  >
                    <Italic size={18} />
                  </button>
                  <button
                    ...
                    title="Underline"
                    onClick={() =>
                      groupedChange('textUnderline', { textUnderline: !textUnderline })
                    }
                  >
                    <Underline size={18} />
                  </button>
                  <button
                    ...
                    title="Strikethrough"
                    onClick={() =>
                      groupedChange('textStrikethrough', { textStrikethrough: !textStrikethrough })
                    }
                  >
                    <Strikethrough size={18} />
                  </button>
                ...
                <div className="toolbar-group">
                  <button
                    ...
                    className={textAlign === 'left' ? 'is-active' : ''}
                    title="Align left"
                    onClick={() => groupedChange('textAlign', { textAlign: 'left' })}
                  >
                    <AlignLeft size={18} />
                  </button>
                  <button
                    ...
                    className={textAlign === 'center' ? 'is-active' : ''}
                    title="Align center"
                    onClick={() => groupedChange('textAlign', { textAlign: 'center' })}
                  >
                    <AlignCenter size={18} />
                  </button>
                  <button
                    ...
                    className={textAlign === 'right' ? 'is-active' : ''}
                    title="Align right"
                    onClick={() => groupedChange('textAlign', { textAlign: 'right' })}
                  >
                    <AlignRight size={18} />
                  </button>
                </div>
        ...
        <>
          ...
          {showChecked ? (
            <label className="checkbox-setting">
              <input
                type="checkbox"
                checked={Boolean(selectedNode.checked)}
                onChange={(event) => groupedChange('checked', { checked: event.target.checked })}
              />
              Checked
            </label>
          ) : null}
      ...
      {canChooseActiveOption ? (
        <section className="property-section">
          <h3>Selection</h3>
          <select
            value={String(selectedActiveIndex)}
            onChange={(event) =>
              groupedChange('activeIndex', { activeIndex: Number(event.target.value) })
            }
          >
      ...
      {showValue ? (
        <label>
          ...
          <input
            type="number"
            min={
              ['progressBar', 'hSlider', 'vSlider', 'volumeSlider'].includes(selectedNode.kind)
                ? 0
                : undefined
            }
            max={
      ...
      {showPlaceholder ? (
        <label>
          ...
          <input
            ...
            onBlur={onNodeChangeEnd}
            onChange={(event) => groupedChange('placeholder', { placeholder: event.target.value })}
          />
        </label>
      ) : null}
      {showTableTextAreas && selectedNode.columns ? (
        <label>
          Columns
          <textarea
            value={selectedNode.columns.join('\n')}
            onBlur={onNodeChangeEnd}
            onChange={(event) =>
              groupedChange('columns', { columns: event.target.value.split('\n') })
            }
          />
        </label>
      ) : null}
      {showTableTextAreas && selectedNode.rows ? (
        <label>
          Rows
          <textarea
            value={selectedNode.rows.join('\n')}
            onBlur={onNodeChangeEnd}
            onChange={(event) => groupedChange('rows', { rows: event.target.value.split('\n') })}
          />
        </label>
      ) : null}
      {!isMultiSelection && (selectedNode.kind === 'icon' || selectedNode.kind === 'iconText') ? (
        <IconPicker
          value={selectedNode.icon ?? 'Plus'}
          onChange={(name) => onNodeChange({ icon: name })}
        />
      ) : null}
      <label className="checkbox-setting">
        <input
          ...
          onChange={(event) => onNodeChange({ locked: event.target.checked })}
        />
        Locked
      </label>
    </div>
        ...
        <div className="settings-dialog-content">
          ...
          <div className="settings-form">
            <div className="setting-row">
              ...
              <select
                value={appAppearance.colorScheme}
                onChange={(event) =>
                  onAppearanceChange({ colorScheme: event.target.value as never })
                }
              >
            ...
            <div className="setting-row">
              ...
              <input
                type="color"
                value={appAppearance.accentColor}
      ...
      >
        ...
        {state.stack.length > 1 ? (
          <div className="context-submenu">
            <button type="button" className="has-submenu">
              Select object<span>›</span>
            </button>
...
function RenameWireframeDialog({
  ...
  const duplicate = wireframes.some(
    (wireframe) =>
      wireframe.id !== state.wireframeId &&
      wireframe.name.trim().toLowerCase() === trimmedName.toLowerCase(),
  );
  const error = !trimmedName
    ? 'Wireframe name is required.'
    : duplicate
      ? 'Another wireframe already uses that name.'
      : '';
  ...
  return (
    <div className="modal-scrim" role="presentation" onMouseDown={onCancel}>
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="rename-wireframe-title"
        onMouseDown={(event) => event.stopPropagation()}
      >
        ...
        <div className="dialog-actions">
          ...
          <button type="button" className="primary" disabled={Boolean(error)} onClick={onSave}>
            Save
          </button>
        </div>
      </section>
  ...
  return (
    <div className="modal-scrim" role="presentation">
      ...
      >
        ...
        <p>{projectName} has unsaved changes. Save before closing?</p>
        <div className="dialog-actions">
...
function IconPicker({ value, onChange }: { value: string; onChange: (name: string) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    const list = q
      ? lucideIconNames.filter((name) => name.toLowerCase().includes(q))
      : lucideIconNames;
    ...
    <div className="icon-picker">
      ...
      {open ? (
        <div className="icon-picker-popover" onMouseDown={(event) => event.stopPropagation()}>
          ...
          {query.trim() &&
          lucideIconNames.filter((n) => n.toLowerCase().includes(query.trim().toLowerCase()))
            .length > 300 ? (
