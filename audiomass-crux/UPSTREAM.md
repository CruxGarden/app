...
## Arrangement tool depth (2026-09-14)

`garden/arrangement.js` prepares scoped edits against a copy of the current native multitrack snapshot, retaining AudioBuffer references. Tracks can be created, named, reordered, removed, muted/soloed and mixed (linear volume 0–1, pan -1–1). Place the current detached waveform as a clip, move it between tracks or timeline positions, trim using source-relative bounds, rename, duplicate, split at an exact timeline position, adjust native fade durations or remove it. Split preserves outer fades and does not apply the native mouse action's zero-crossing snap. Master volume and guarded native history are exposed. Fades must be zero or at least 5 ms, and their combined length must fit the clip, matching native constraints without silently clamping requested edits. Limits retain 128 tracks/2000 clips and bound tool-authored timelines to 4000 seconds. Source PCM remains unmodified.

The added `gardenApplyArrangement` seam in `src/multitrack.js` uses native `restoreState`/`pushState` for one synchronous, validated transaction and native Undo. Validation and editing logic stay in Garden files; no whole-document replacement command is exposed to agents. It requires a detached waveform to avoid stale clip-editor bindings. Inspection supplies an arrangementHash over track/clip metadata and audio contents; manual changes during validation invalidate the command. Native controls remain editable, and unrelated fields and view state are retained. Arrangement changes stop playback through native restore. Undo remains the native shared history, not a separate Garden undo stack.

Reload now reuses decoded source buffers for identical audio descriptors within the same restore, so duplicate/trimmed clips keep sharing source PCM. This cache does not persist across edits or retain superseded buffers. The separate waveform editor still loads through its native import API.

Upgrade checks: verify native snapshot fields and buffer identity semantics, restoreState rendering/selection/crossfade cleanup, pushState native Undo/Redo, native mix volume/pan/fade behavior and the arrangement desktop acceptance. Markers, opening clips in the waveform editor via tools, advanced effects/analysis, crossfade authoring, tempo/grid and session interchange remain native controls pending later tool families. This pass is not whole-app certification.
