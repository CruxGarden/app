...
export function usePublishing(): UsePublishingProps {
	...
	const currentStories = React.useRef(stories);
	currentStories.current = stories;

	return {
		publishArchive: React.useCallback(async () => {
			await window.gardenTwine?.garden.flush();
			return publishArchive(currentStories.current, getAppInfo());
		}, [stories]),
		proofStory: React.useCallback(
			async storyId => {
				await window.gardenTwine?.garden.flush();
				const story = storyWithId(currentStories.current, storyId);
