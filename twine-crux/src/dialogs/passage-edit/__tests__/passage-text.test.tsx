...
describe('<PassageText>', () => {
	...
	it('marks Garden dirty before the native debounced update and releases the pending draft afterward', () => {
		const changed = jest.fn();
		const pending = new Set<symbol>();
		const onChange = jest.fn();
		window.gardenTwine = {garden: {changed}, pending} as any;
		try {
			renderComponent({onChange});
			fireEvent.change(
				screen.getByLabelText('dialogs.passageEdit.passageTextEditorLabel'),
			...
			expect(onChange).toHaveBeenCalledWith('Latest draft');
