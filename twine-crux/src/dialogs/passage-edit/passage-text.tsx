...
export const PassageText: React.FC<PassageTextProps> = props => {
	...
	const gardenDraftId = React.useRef(Symbol('passage-draft'));
	...
	const handleLocalChangeText = React.useCallback(
		(text: string) => {
			...
			setLocalText(text);
			window.gardenTwine?.pending.add(gardenDraftId.current);
			window.gardenTwine?.garden.changed();
			...
			onChangeTimeout.current = window.setTimeout(() => {
				...
				window.gardenTwine?.pending.delete(gardenDraftId.current);
			}, 1000);
		},
