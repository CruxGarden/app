...
export const BuildActions: React.FC<BuildActionsProps> = ({story}) => {
	...
	const currentStory = React.useRef(story);
	...
	async function handleExportAsTwee() {
		if (!story) {
		...
		resetErrors();
		try {
			if (window.gardenTwine) await window.gardenTwine.garden.flush();
			const latest = currentStory.current;
			if (!latest || latest.id !== story.id)
				throw new Error('The selected story changed. Export it again.');
			saveTwee(storyToTwee(latest), storyFileName(latest, '.twee'));
		} catch (error) {
			setPublishError(error as Error);
		}
	}
