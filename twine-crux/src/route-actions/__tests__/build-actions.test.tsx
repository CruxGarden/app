...
describe('<BuildActions>', () => {
	...
	it('exports the latest story after Garden finishes its pending draft', async () => {
		usePublishingMock.mockReturnValue({});
		useStoryLaunchMock.mockReturnValue({});
		let finish!: () => void;
		window.gardenTwine = {
		...
		try {
			const initial = fakeStory();
			const {rerender} = renderComponent({story: initial});
			fireEvent.click(screen.getByText('routeActions.build.exportAsTwee'));
			expect(saveTweeMock).not.toHaveBeenCalled();
			const latest = {...initial, name: 'Latest saved draft'};
			rerender(<BuildActions story={latest} />);
			await act(async () => finish());
			expect(saveTweeMock).toHaveBeenCalledWith(
				storyToTwee(latest),
				storyFileName(latest, '.twee')
			);
		} finally {
			delete (window as any).gardenTwine;
		}
	});
