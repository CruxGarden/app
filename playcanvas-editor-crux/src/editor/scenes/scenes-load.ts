...
editor.once('load', () => {
    ...
    const goToProject = function () {
        history.replaceState(null, 'Editor', `${window.location.pathname}${window.location.search}`);
    ...
    editor.on('scene:load', (id: number | string, uniqueId: number | string) => {
        ...
        if (pushState) {
            if (history.length === 1 && window.location.pathname.startsWith('/editor/scene/')) {
                history.replaceState(null, 'Editor', `${window.location.pathname}?sceneId=${id}`);
            } else {
                history.pushState(null, 'Editor', `${window.location.pathname}?sceneId=${id}`);
            }
        }
