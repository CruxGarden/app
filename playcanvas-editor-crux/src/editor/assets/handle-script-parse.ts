...
editor.once('load', () => {
    ...
    workerClient.once('init', async () => {
        ...
        const handleParseResult = (res, asset, callback, inEditor) => {
            if (inEditor) {
            ...
            // Garden owns the asset document; the parser remains upstream's sandboxed parser.
            asset.set('data.scripts', res.scripts || {});
            asset.set('data.loading', false);
            if (inEditor) editor.call('status:clear');
            callback?.(null, res);
        };

        const postUrl = (asset) => asset.get('file.url');
        ...
        workerClient.on('attributes:parse', (guid, scripts, scriptsInvalid) => {
            ...
            handleParseResult(res, asset, callback, inEditor);
        });
...
onmessage = (e) => {
    ...
    worker.postMessage({ engine, script });
                ...
                channel.port1.onmessage = (evt) => {
                    ...
                    handleParseResult(result, asset, callback, inEditor);
                };
