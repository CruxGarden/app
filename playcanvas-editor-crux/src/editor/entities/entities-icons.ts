...
editor.once('load', () => {
    ...
    editor.once('viewport:load', (application) => {
        ...
        textureNames.forEach((textureName) => {
            ...
            // Local, source-defined marker; no dependency on hosted icon files.
            const label = textureName.slice(0, 1).toUpperCase();
            img.src = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><circle cx="16" cy="16" r="14" fill="white"/><text x="16" y="22" text-anchor="middle" font-family="sans-serif" font-size="20" fill="black">${label}</text></svg>`)}`;
        });
