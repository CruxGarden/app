...
test('accepts an empty project and a calendar', () => {
  ...
  validateProject(doc([]));
  validateProject(
    doc([event(), event({ id: 'x', end: '', allDay: true, color: '' })], { date: '' }),
  );
});
test('rejects other apps, bad times and bad events', () => {
  ...
  assert.throws(() => validateProject(doc([event({ start: '2026-02-30T10:00:00' })])), /start/);
  assert.throws(() => validateProject(doc([event({ end: '2026-09-15T09:00:00' })])), /end/);
  assert.throws(() => validateProject(doc([event(), event()])), /Duplicate event id/);
});
