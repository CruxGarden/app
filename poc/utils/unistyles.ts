 ...
 *
 * Integrates with CruxGarden theming system
 */
...
/**
 * Convert DesignTokens to Unistyles theme
 */
export function tokensToUnistylesTheme(tokens: DesignTokens) {
  return {
    ...
    fonts, // Static font mapping
    gap: (v: number) => v * 8, // Utility function for spacing
  };
}

// Get default tokens and create initial theme
const defaultTokens = getDefaultTokens();
const defaultTheme = tokensToUnistylesTheme(defaultTokens);

// Create light and dark themes (will be updated by ThemeProvider)
