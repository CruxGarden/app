...
export default class RootStore {
  ...
  async init() {
    ...
    // Crux Garden saves the song itself when the app is framed in a Workshop
    if (window.parent === window) this.autoSaveService.startAutoSave()
...
async function loadMetronomeSoundFontData() {
  ...
  // Crux Garden: vendored so the metronome loads offline (public/soundfonts)
  const soundFontURL = new URL("soundfonts/A320U_drums.sf2", document.baseURI).href
