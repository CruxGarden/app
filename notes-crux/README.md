...
Crux settings → Public notebook layout chooses `single-page` (the default searchable reader) or `separate-pages` (individual HTML pages, usable without JavaScript). The choice is saved as `layout` in notebook/publish.json and travels with Growth, Tasks and exports. Share updates the website. Separate pages use /notes/<full-note-path>/, including .md; switching layout or renaming a note changes its URL. Links to unselected notes remain plain text. Older Crux copies need an updated reader before this setting is available; app sources are never automatically replaced.
