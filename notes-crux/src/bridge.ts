...
export function request<T>(op: string, fields: Record<string, unknown> = {}): Promise<T> {
  return new Promise((resolve, reject) => {
    if (window.parent === window)
      return reject(new Error('Open this notebook inside Crux Garden.'));
    const id = `notebook-${++sequence}`;
    ...
    function receive(event: MessageEvent) {
      if (
        ...
        event.data?.type !== 'crux:notebook:result' ||
        event.data.id !== id
      )
        return;
    ...
    // A narrow protocol, containing notebook data only. Never sends credentials.
    window.parent.postMessage({ type: 'crux:notebook', id, op, ...fields }, '*');
  });
}
