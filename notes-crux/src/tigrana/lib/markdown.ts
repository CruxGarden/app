...
export function markdownToHtml(markdown: string, options: MarkdownOptions = {}) {
  ...
  const emitListItem = (indent: number, type: ListLevel['type'], openItemHtml: string) => {
    ...
    if (listInternalBlankRun > 0 && top.lastLiIndex >= 0) {
      html[top.lastLiIndex] = html[top.lastLiIndex].replace(
        /^<li/,
        '<li data-separator-after="true"',
      );
    }
    listInternalBlankRun = 0;
  ...
  while (i < lines.length) {
    ...
    if (heading) {
      ...
      flushBlankParagraphs();
      html.push(
        `<h${heading[1].length}>${inlineMarkdownToHtml(heading[2], options)}</h${heading[1].length}>`,
      );
      i += 1;
      continue;
    }
    ...
    if (standaloneImage) {
      ...
      const resolvedSrc = options.resolveImageSrc?.(src) ?? src;
      html.push(
        `<img src="${escapeHtml(resolvedSrc)}" alt="${escapeHtml(alt)}" data-markdown-src="${escapeHtml(src)}" />`,
      );
      i += 1;
      continue;
    }
  ...
  blocks.forEach((block, index) => {
    ...
    if (/^h[1-6]$/.test(tag)) {
      ...
      const joined = segments
        .map((segment, idx) => (idx < lastIndex ? `${segment}  ` : segment))
        .join('\n');
      ...
      if (allRows.length > 0) {
        ...
        tableLines.push(
          '| ' + firstCells.map((c) => inlineHtmlToMarkdown(c).trim()).join(' | ') + ' |',
        );
        tableLines.push('| ' + firstCells.map(() => '---').join(' | ') + ' |');
        for (const row of allRows.slice(1)) {
          const cells = Array.from(row.children);
          tableLines.push(
            '| ' + cells.map((c) => inlineHtmlToMarkdown(c).trim()).join(' | ') + ' |',
          );
        }
...
function serializeTigranaHtmlTable(table: Element) {
  ...
  const writeRow = (row: Element, indent: string) => {
    ...
    Array.from(row.children).forEach((cell, cellIndex) => {
      const tag =
        headerColumn && cellIndex === 0 ? 'th' : cell.tagName.toLowerCase() === 'th' ? 'th' : 'td';
...
function serializeTableCellHtml(cell: Element) {
  ...
  clone.querySelectorAll('[class]').forEach((element) => {
    ...
    const kept = className
      .split(/\s+/)
      .filter((name) => !/^ProseMirror/.test(name) && name !== 'selectedCell');
