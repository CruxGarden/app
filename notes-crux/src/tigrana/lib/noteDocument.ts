...
export function readNoteCreatedAt(markdown: string): number | null {
  const field = readNoteDocument(markdown, '').frontmatterFields.find(
    (candidate) => candidate.key === 'created_at',
  );
...
export function createNoteDocument({
  ...
  frontmatter,
  body,
}: NoteDocumentContent): NoteDocument {
  ...
  return buildNoteDocument({
    ...
    frontmatterError: validationError
      ? `This note has malformed frontmatter: ${validationError}.`
      : null,
  });
}

export function reviseNoteDocument(
  document: NoteDocument,
  revision: NoteDocumentRevision,
): NoteDocument {
...
export function updateNoteDocumentFrontmatterField(
  document: NoteDocument,
...
function parseMarkdown(markdown: string, includeCanonicalMarkdown = true): ParsedMarkdown {
  ...
  if (closingIndex === -1) {
    return {
      ...
      frontmatter: '',
      frontmatterError:
        'This note starts with frontmatter, but the closing --- line is missing. Opened as raw Markdown.',
    };
  }
...
function readFrontmatterFields(frontmatter: string): FrontmatterField[] {
  ...
  return lines.flatMap((line, index) => {
    ...
    const nextTopLevelIndex = lines.findIndex(
      (nextLine, nextIndex) => nextIndex > index && /^\S/.test(nextLine),
    );
    const blockEnd = nextTopLevelIndex === -1 ? lines.length : nextTopLevelIndex;
    const hasNestedContent = lines
      .slice(index + 1, blockEnd)
      .some((nextLine) => Boolean(nextLine.trim()) && /^\s/.test(nextLine));
    return [
      {
...
function normalizeFrontmatterClosingFence(markdown: string) {
  ...
  const closingIndex = lines.findIndex((line, index) => index > 0 && line.trim() === '---');
  const smartDashIndex = lines.findIndex(
    (line, index) => index > 0 && isSmartDashFence(line.trim()),
  );
  if (smartDashIndex === -1 || (closingIndex !== -1 && closingIndex < smartDashIndex))
    return markdown;
