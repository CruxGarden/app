...
export function setEditorSpellcheck(editor: SpellcheckEditor | null, enabled: boolean) {
  ...
  editor.setOptions({
    editorProps: {
      ...editorProps,
      attributes:
        typeof currentAttributes === 'function'
...
export function cacheCurrentNoteEditorState(
...
export function getEditorDocumentLoadAction({
  ...
  if (currentHistoryKey && nextHistoryKey && currentHistoryKey === nextHistoryKey)
    return 'preserve';
  return 'load';
}
  ...
  return (
    <NodeViewWrapper as="span" className="image-resizable-wrapper">
      ...
      >
        ...
        />
        {selected && <span className="image-resize-handle" onMouseDown={handleResizeStart} />}
      </span>
    </NodeViewWrapper>
  );
}
...
function parseCellColwidth(element: HTMLElement) {
  ...
  const raw =
    ...
    /width\s*:\s*(\d+(?:\.\d+)?)px/i.exec(col?.getAttribute('style') ?? '')?.[1] ??
    null;
...
class TableControlsNodeView implements NodeView {
  ...
  constructor(
    ...
    this.rowHandle = this.dom.appendChild(
      createTableToolButton('Row options', tableIconSvg('ellipsisVertical', 16)),
    );
    ...
    this.columnHandle = this.dom.appendChild(
      createTableToolButton('Column options', tableIconSvg('ellipsis', 16)),
    );
    ...
    this.addColumnEdgeButton = this.dom.appendChild(
      createTableToolButton('Add column', tableIconSvg('plus', 14)),
    );
    ...
    this.addRowEdgeButton = this.dom.appendChild(
      createTableToolButton('Add row', tableIconSvg('plus', 14)),
    );
  ...
  findSelectionCellElement() {
    ...
    const element =
      domAtSelection instanceof Element ? domAtSelection : domAtSelection.parentElement;
  ...
  handleMouseLeave = () => {
    ...
    window.setTimeout(() => {
      if (
        this.dom.matches(':hover') ||
        this.rowHandle.matches(':hover') ||
        this.columnHandle.matches(':hover')
      )
        return;
  ...
  applyHandleVisibility() {
    ...
    const isFullRowSelection = Boolean(
      rowRange && colRange && colRange.start === 0 && colRange.end === lastCol,
    );
    const isFullColumnSelection = Boolean(
      rowRange && colRange && rowRange.start === 0 && rowRange.end === lastRow,
    );
  ...
  positionEdgeButtons() {
    ...
    const isFullRowSelection = Boolean(
      ...
      colRange &&
      colRange.start === 0 &&
      colRange.end === lastCol &&
      rowRange.end - rowRange.start < map.height - 1,
    );
    const isFullColumnSelection = Boolean(
      rowRange &&
      colRange &&
      rowRange.start === 0 &&
      rowRange.end === lastRow &&
      colRange.end - colRange.start < map.width - 1,
    );
    ...
    const columnVisible = hoverInLastCol || cursorInLastCol || this.addColumnButtonHovered;
    const rowVisible = hoverInLastRow || cursorInLastRow || this.addRowButtonHovered;
  ...
  getColumnWidths() {
    ...
    return Array.from({ length: map.width }, (_value, index) => {
      ...
      const attrWidth = parseFloat(
        col?.getAttribute('data-width') ?? col?.getAttribute('width') ?? '',
      );
      ...
      return typeof colwidth === 'number' && Number.isFinite(colwidth)
        ? colwidth
        : RICH_TABLE_DEFAULT_COLUMN_WIDTH;
    });
  }
  ...
  runTableCommandAt(
    ...
    options: { rebalanceColumns?: number } = {},
  ) {
    const pos = this.resolvePos();
    ...
    const selection = Selection.near(
      state.doc.resolve(Math.min(absoluteCellPos + 1, state.doc.content.size)),
    );
  ...
  selectRow(row: number, options: { preserveHandlePosition?: boolean } = {}) {
    ...
    this.preserveScrollAround(() => {
      this.view.dispatch(
        this.view.state.tr.setSelection(CellSelection.create(this.view.state.doc, anchor, head)),
      );
  ...
  insertColumn(column: number, direction: 'left' | 'right') {
    this.runTableCommandAt(0, column, direction === 'left' ? addColumnBefore : addColumnAfter, {
      rebalanceColumns: 1,
    });
    this.closeAxisMenu();
  }
    ...
    this.preserveScrollAround(() => {
      this.view.dispatch(
        this.view.state.tr.insert(
          rowInfo.pos + rowInfo.node.nodeSize,
          rowInfo.node.copy(rowInfo.node.content),
        ),
      );
      this.normalizeRichHeaderCells();
      this.refreshSelectionActive();
  ...
  rebalanceColumns(delta: number) {
    ...
    const total =
      ...
      map.width * RICH_TABLE_DEFAULT_COLUMN_WIDTH;
    const widths = Array.from(
      { length: map.width },
      (_value, index) => currentWidths[index] ?? RICH_TABLE_DEFAULT_COLUMN_WIDTH,
    );
    if (delta > 0) {
      const target = total / map.width;
      const scaled = widths.map((width) =>
        Math.max(RICH_TABLE_MIN_COLUMN_WIDTH, width - target / Math.max(map.width - 1, 1)),
      );
  ...
  normalizeRichHeaderCells() {
    ...
    if (!table) return;
    const tr = this.view.state.tr;
    applyHeaderCellsToTransaction(
      tr,
      table,
      pos,
      Boolean(table.attrs.headerRow),
      Boolean(table.attrs.headerColumn),
    );
  ...
  copyTable(label: HTMLSpanElement, iconSlot: HTMLSpanElement) {
    ...
    void writeRichClipboard(html, markdown)
      .then(() => {
  ...
  focusWithoutScroll() {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (anchorCellPos != null && headCellPos != null) {
        const curAnchor = (current as Selection & { $anchorCell?: { pos: number } }).$anchorCell
          ?.pos;
  ...
  appendTableMenuActions(menu: HTMLDivElement) {
    const copyButton = menu.appendChild(
      createTableMenuButton(tableIconSvg('copy', 14), 'Copy table'),
    );
    ...
    menu
      .appendChild(createTableMenuButton(tableIconSvg('plus', 14), 'Add blank line after table'))
      .addEventListener('click', () => this.insertParagraphAfterTable());

    const convertButton = menu.appendChild(
      createTableMenuButton(
        tableIconSvg('table', 14),
        isRichTableNode(this.node)
          ? 'Convert to Markdown table (loses formatting)'
          : 'Convert to HTML table for more options',
      ),
    );
    ...
    const deleteButton = menu.appendChild(
      createTableMenuButton(tableIconSvg('trash', 14), 'Delete table'),
    );
  ...
  appendTableOptionsSubmenu(menu: HTMLDivElement) {
    ...
    const trigger = wrap.appendChild(
      createTableSubmenuButton(tableIconSvg('table', 14), 'Table options'),
    );
  ...
  openAxisMenu(axis: TableAxis, index: number, anchorRect: DOMRect) {
    ...
    if (axis === 'row') {
      ...
      const headerButton = this.axisMenu.appendChild(
        createTableMenuSwitchButton(
          'Header row',
          isRichTable ? Boolean(this.node.attrs.headerRow) : true,
          !isRichTable,
        ),
      );
      if (isRichTable) {
      ...
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('arrowUp', 14), 'Insert row above'))
        .addEventListener('click', () => this.insertRow(index, 'above'));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('arrowDown', 14), 'Insert row below'))
        .addEventListener('click', () => this.insertRow(index, 'below'));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('copy', 14), 'Duplicate row'))
        .addEventListener('click', () => this.duplicateRow(index));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('xCircle', 14), 'Clear row contents'))
        .addEventListener('click', () => this.clearRow(index));
      const deleteButton = this.axisMenu.appendChild(
        createTableMenuButton(tableIconSvg('trash', 14), 'Delete row'),
      );
      deleteButton.classList.add('danger-item');
      ...
      this.axisMenu.appendChild(createTableMenuSeparator());
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('arrowLeft', 14), 'Insert column left'))
        .addEventListener('click', () => this.insertColumn(index, 'left'));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('arrowRight', 14), 'Insert column right'))
        .addEventListener('click', () => this.insertColumn(index, 'right'));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('copy', 14), 'Duplicate column'))
        .addEventListener('click', () => this.duplicateColumn(index));
      this.axisMenu
        .appendChild(createTableMenuButton(tableIconSvg('xCircle', 14), 'Clear column contents'))
        .addEventListener('click', () => this.clearColumn(index));
      const deleteButton = this.axisMenu.appendChild(
        createTableMenuButton(tableIconSvg('trash', 14), 'Delete column'),
      );
      deleteButton.classList.add('danger-item');
...
function getRowCellInfos(table: ProseMirrorNode, tablePos: number, row: number) {
  ...
  return Array.from({ length: map.width }, (_value, column) => {
    ...
    (info, index, all) => info.node && all.findIndex((other) => other.pos === info.pos) === index,
  );
}
...
function applyColumnWidthsToTransaction(
  tr: Transaction,
...
function normalizeWidthsToTotal(widths: number[], total: number) {
  ...
  return clamped.map((width) =>
    Math.max(RICH_TABLE_MIN_COLUMN_WIDTH, Math.round((width / sum) * total)),
  );
}
...
const TableWithControls = Table.extend({
  addAttributes() {
    return {
      ...
      tigranaTable: {
        ...
        renderHTML: (attributes) =>
          attributes.tigranaTable ? { 'data-tigrana-table': 'true' } : {},
      },
      headerRow: {
        default: true,
        parseHTML: (element) => element.getAttribute('data-header-row') !== 'false',
        renderHTML: (attributes) =>
          attributes.tigranaTable
  ...
  addNodeView() {
    return ({ node, view, getPos }) =>
      new TableControlsNodeView(node, this.options.cellMinWidth, view, getPos);
  },
});
  ...
  if (firstRow) {
    for (let cellIndex = 0, col = 0; cellIndex < firstRow.childCount; cellIndex += 1) {
      ...
      const colwidth = Array.isArray(cell.attrs.colwidth)
        ? (cell.attrs.colwidth as unknown[])
        : null;
      for (let span = 0; span < colspan; span += 1, col += 1) {
        const widthValue = colwidth?.[span];
        const width =
          typeof widthValue === 'number' && Number.isFinite(widthValue) ? widthValue : null;
        ...
        if (isRichTableNode(node) && width)
          nextDOM.setAttribute('data-width', String(Math.max(width, cellMinWidth)));
...
export function NotesEditor({
    ...
    try {
      initialContentRef.current = {
        ...
        html: markdownToHtml(content, {
          resolveImageSrc: (src) => resolveNotebookImageSrc(workspace, src),
        }),
      };
    } catch (error) {
  ...
  const handleSlashKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowDown') {
        ...
        setSlash((current) =>
          current
            ? { ...current, selected: (current.selected + 1) % currentCommands.length }
            : current,
        );
        return true;
      }
      ...
      if (event.key === 'ArrowUp') {
        ...
        setSlash((current) =>
          current
            ? {
                ...current,
                selected: (current.selected - 1 + currentCommands.length) % currentCommands.length,
              }
            : current,
        );
        return true;
      ...
      handleDOMEvents: {
        ...
        mousedown(view, event) {
          ...
          const lastBlockBottom =
            lastBlock?.getBoundingClientRect().bottom ?? editorElement.getBoundingClientRect().top;
      ...
      handlePaste(view, event) {
        ...
        void getBestClipboardImageFile(file ?? htmlFile)
          .then((imageFile) => {
            ...
            return insertNativeClipboardImage(
              view,
              workspace,
              pasteNotePath,
              () => notePathRef.current,
      ...
      capture() {
        ...
        const parseOptions = {
          ...
          ruleFromNode(node: Node) {
            ...
            if (node.nodeName === 'BR' && node === node.parentNode?.lastChild)
              return { ignore: true };
            return null;
          },
        };
        const visible = ProseMirrorDOMParser.fromSchema(editor.schema).parse(
          editor.view.dom,
          parseOptions,
        );
        if (!visible.eq(editor.state.doc)) {
          editor.view.dispatch(
            editor.state.tr.replaceWith(0, editor.state.doc.content.size, visible.content),
          );
        }
  ...
  useEffect(() => {
    ...
    if (loadAction === 'unchanged') {
      ...
      if (
        nextHistoryKey &&
        previousHistoryKey &&
        nextHistoryKey !== previousHistoryKey &&
  ...
  useEffect(() => {
    ...
    const preservedScrollTop =
      scrollSurface instanceof HTMLElement ? scrollSurface.scrollTop : null;
    const restoreScroll = () => {
      if (
  ...
  const selectMatchFromList = useCallback(
    (matches: Array<{ from: number; to: number }>, index: number) => {
  ...
  const applyEditorCommand = useCallback(
    (request: EditorCommandRequest) => {
      ...
      if (request.command === 'insertText') {
        ...
        if (typeof request.selectionFrom === 'number') {
          ...
          editor
            .chain()
            .focus()
            .insertContentAt({ from, to }, request.src)
            .setTextSelection(from + request.src.length)
            .run();
          return;
        }
      ...
      switch (request.command) {
        ...
        case 'h6':
          chain
            .toggleHeading({ level: Number(request.command.slice(1)) as 1 | 2 | 3 | 4 | 5 | 6 })
            .run();
          break;
        case 'bulletList':
        ...
        <div className={replaceOpen ? 'note-find-bar has-replace' : 'note-find-bar'}>
          ...
          {replaceOpen ? (
            <div className="note-replace-row">
              ...
              <button
                type="button"
                disabled={!findMatches.length || !editable}
...
function handleEditorTabKeyDown(editor: Editor, event: KeyboardEvent) {
  ...
  const listItemName = editor.isActive('taskItem')
    ? 'taskItem'
    : editor.isActive('listItem')
      ? 'listItem'
      : null;
...
function isTableChromeTarget(target: EventTarget | null) {
  return (
    ...
    target.closest(
      '.table-axis-handle, .table-edge-add, .table-context-menu, .table-column-resize-handle',
    ) != null
  );
}
...
export function FormattingBubbleMenu({
  ...
  const eligible = (() => {
    ...
    if (activeElement instanceof HTMLElement && activeElement.closest('.note-find-bar'))
      return false;
    const { selection } = editor.state;
...
function scrollEditorPositionIntoView(editor: Editor, position: number) {
  ...
  const targetTop =
    scrollContainer.scrollTop + coords.top - containerRect.top - containerRect.height * 0.42;
...
function handleEmptyListItemDelete(view: EditorView, event: KeyboardEvent) {
  ...
  const nextTextPosition = findTextSelectionPosition(
    tr.doc,
    deleteFrom,
    schema.nodes.paragraph?.name ?? 'paragraph',
  );
...
export function handleNestedListBoundaryDelete(view: EditorView, event: KeyboardEvent) {
  ...
  const mergedParagraph = parentParagraph.copy(
    parentParagraph.content.append(childParagraph.content),
  );
  const replacementChildren: ProseMirrorNode[] = [];
  for (let index = 0; index < nestedListIndex; index += 1) {
    replacementChildren.push(
      index === parentParagraphIndex ? mergedParagraph : parentItem.child(index),
    );
  }

  const childBlocks = Array.from({ length: childItem.childCount - 1 }, (_, index) =>
    childItem.child(index + 1),
  );
  const remainingSiblings = Array.from({ length: nestedList.childCount - 1 }, (_, index) =>
    nestedList.child(index + 1),
  );
  ...
  const tr = view.state.tr.replaceWith(
    parentItemFrom,
    parentItemFrom + parentItem.nodeSize,
    replacement,
  );
...
function findForwardNestedListBoundary($cursor: ResolvedPos): NestedListBoundary | null {
  ...
  if (!$cursor.parent.isTextblock || !isListNode(nestedList) || !isListItemNode(childItem))
    return null;
  return {
    childItem,
    nestedList,
...
function findBackwardNestedListBoundary($cursor: ResolvedPos): NestedListBoundary | null {
  ...
  if (!child || $cursor.index(child.depth) !== 0 || $cursor.index(child.parentDepth) !== 0)
    return null;
  ...
  if (!parentParagraph?.isTextblock || parentItem.child(nestedListIndex) !== child.parentNode)
    return null;
  return {
...
export function handleSameLevelListItemBackspace(view: EditorView, event: KeyboardEvent) {
  ...
  if (
    ...
    !$cursor.parent.textContent.trim()
  )
    return false;
  ...
  if (
    ...
    !previousParagraph?.isTextblock ||
    !currentParagraph?.isTextblock
  )
    return false;

  const previousBlocks = Array.from({ length: previousItem.childCount - 1 }, (_, index) =>
    previousItem.child(index + 1),
  );
  const currentBlocks = Array.from({ length: item.node.childCount - 1 }, (_, index) =>
    item.node.child(index + 1),
  );
...
export function handleOutermostListItemBackspace(view: EditorView, event: KeyboardEvent) {
  ...
  const item = findListItemAtSelection($cursor);
  if (!item || $cursor.index(item.depth) !== 0 || $cursor.index(item.parentDepth) !== 0)
    return false;
  ...
  const handled = liftListItem(item.node.type)(view.state, (transaction) =>
...
function findNextEmptyTaskItem($from: ResolvedPos, current: ListItemRange): ListItemRange | null {
  ...
  const nextSibling =
    itemIndex < current.parentNode.childCount - 1 ? current.parentNode.child(itemIndex + 1) : null;
  ...
  if (!nextList || !nextItem || nextItem.type.name !== 'taskItem' || nextItem.textContent.trim())
    return null;
...
function isPartialSelectionWithinSingleListLine(selection: Selection) {
  ...
  const endItem = findListItemAtSelection(
    selection.$from.doc.resolve(Math.max(selection.from, selection.to - 1)),
  );
  if (!startItem || !endItem || startItem.from !== endItem.from) return false;

  const startBlock = findTextblockRangeAtPos(selection.$from);
  const endBlock = findTextblockRangeAtPos(
    selection.$from.doc.resolve(Math.max(selection.from, selection.to - 1)),
  );
...
export function getTaskLineCutDeleteRange(selection: Selection) {
  ...
  const endsAtLineEnd =
    selection.to === lastTextEnd || selection.to === lastItem.from + lastItem.node.nodeSize;
...
function normalizeTableClipboardHtml(html: string) {
  ...
  container
    .querySelectorAll<HTMLElement>(
      '[data-node-view-wrapper], [data-node-view-content], [data-node-view-content-react]',
    )
    .forEach((element) => {
...
function resolveNotebookImageSrc(_workspace: string, src: string) {
  // Local images are hydrated via the scoped Garden bridge below.
...
async function readClipboardImageFile() {
  ...
  for (const item of items) {
    const type =
      item.types.find(isRenderableImageType) ??
...
async function previewSrcForNotebookImage(workspace: string, src: string) {
  if (!workspace || isExternalImageSrc(src) || src.startsWith('data:')) return src;
